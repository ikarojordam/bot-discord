...
async function cleanupRoles(guild, bot) {
  ...
  const rolesToDelete = guild.roles.cache.filter(r =>
    r.id !== guild.roles.everyone.id && r.name !== '.' && !r.managed
  );
...
// Helper comum: verifica permissões ANTES
...
// Helper: cria cargos sequencialmente (evita rate limit)
...
async function setupLojaServer(guild, onProgress = null) {
  ...
  // 🔧 FIX: verifica permissões antes
  checkSetupPermissions(bot);

  // 🔧 FIX: bloqueia setups concorrentes
  ...
  try {
    ...
    Object.assign(cfg, {
      ...
      server_type: 'loja', ticket_titulo: '🎟・Central de Atendimento', ticket_descricao: 'Selecione o tipo de atendimento.'
    });
...
async function setupComunidadeServer(guild, onProgress = null) {
  ...
  try {
    ...
    Object.assign(cfg, {
      ...
      server_type: 'comunidade', ticket_titulo: '🎟・Central de Suporte', ticket_descricao: 'Selecione o tipo.'
    });
    await setConfig(guild.id, cfg);
    await patchSettings(guild.id, {
  ...
  try {
    ...
    if (!skipPosting) {
      ...
      const staticEmbeds = [
        ...
        { ch: '🎥・fila-streamer', t: '🎥 Fila Streamer', c: '#9146FF', d: 'Streamers ao vivo!' }
      ];
      ...
      const extras = [
        { ch: '🏦・bancos-proibido', t: '🏦 Bancos Aceitos', c: '#22c55e', d: '✅ Nubank, Inter, C6, Itaú, Bradesco, Santander, Caixa, BB, PagBank, PicPay, Mercado Pago.\n⚠️ PIX no nome do titular apostador.' },
        { ch: '🛬・invites', t: '🛬 Convites', c: '#5865F2', d: '**Convide amigos** e ganhe coins!\n• 5 = 100 coins\n• 10 = 300 coins\n• 25 = 1000 + cargo' },
        { ch: '💬・chat-geral', t: '💬 Chat Geral', c: '#5865F2', d: 'Bem-vindo! Fale com a galera.' },
        { ch: '🏆・wins', t: '🏆 Mural de Wins', c: '#FFD700', d: 'Poste suas vitórias!' },
        { ch: '🎥・clips', t: '🎥 Clips', c: '#FF69B4', d: 'Poste seus melhores clips!' },
        { ch: '🦊・[nome da sua org]-cargos', t: '🦊 Cargos da Org', c: '#FFA500', d: 'Cargos exclusivos.' },
        ...
        { ch: '💰・pagamentos', t: '💰 Pagamentos', c: '#22c55e', d: 'Prêmios em PIX ou coins.' },
        { ch: '🎁・avisos-ranking', t: '🎁 Ranking', c: '#FFD700', d: 'Toda segunda postamos o ranking.\n🥇 500 coins\n🥈 250 coins\n🥉 100 coins' },
        { ch: '🏆・premiações', t: '🏆 Premiações', c: '#FFD700', d: 'Registro de premiações.' },
        { ch: '🟢・live-on', t: '🟢 Ao Vivo!', c: '#00FF00', d: 'Avisem quando estiver AO VIVO!' },
        { ch: '📣・divulgacão', t: '📣 Divulgação', c: '#5865F2', d: 'Divulgue vídeos e redes.' },
        { ch: '🚫・exposed-mob', t: '🚫 Exposed Mobile', c: '#FF5555', d: 'Banidos por usar emulador no mobile.' },
        { ch: '・chat-streamer', t: '🎥 Chat Streamers', c: '#9146FF', d: 'Canal privado.' },
      ...
      const vagas = [
        { ch: '👨🏻・vagas-suporte', t: '👨🏻 VAGAS — SUPORTE', c: '#5865F2', d: '**REQ:** 15+ anos, ativo, paciência.\n**SAL:** R$ 0,10/atendimento\n**CAND:** Ticket [VAGA SUPORTE]' },
        { ch: '🔎・seja-analista', t: '🔎 VAGAS — ANALISTA', c: '#00AAFF', d: '**REQ:** 16+, conhecer FF, imparcial.\n**SAL:** R$ 0,05/análise\n**CAND:** Ticket [VAGA ANALISTA]' },
        { ch: '💸・seja-adm', t: '💸 VAGAS — ADM', c: '#FF5555', d: '**REQ:** 18+, experiência, 4h/dia.\n**SAL:** A combinar\n**CAND:** Ticket [VAGA ADM]' },
        { ch: '🎥・seja-influencer', t: '🎥 VAGAS — INFLUENCER', c: '#FF69B4', d: '**REQ:** Canal +500 inscritos\n**BENEF:** Cargo + 500 coins/mês\n**CAND:** Ticket [VAGA INFLUENCER]' },
        { ch: '🤝🏻・pedir-parceria', t: '🤝🏻 PARCERIAS', c: '#9B59B6', d: '**REQ:** +100 membros, boa moderação.\n**PROPOSTA:** Ticket [PARCERIA]' },
        { ch: '📮・suporte', t: '📮 CENTRAL DE SUPORTE', c: '#5865F2', d: 'Tipos:\n> 🛠️ Suporte\n> 🎁 Receber Evento\n> 💸 Reembolso\n> 🛡️ Vaga Mediador\n> 🎥 Vaga Influencer' },
        { ch: '🥂・eventos', t: '🥂 EVENTOS', c: '#FFD700', d: '**Ativos:**\n> Torneio semanal\n> Sorteio mensal\n> Aposta relâmpago' },
        { ch: '💎・solicitar-analista', t: '💎 SOLICITAR ANALISTA', c: '#00AAFF', d: 'Motivos:\n> Oponente usa hack\n> Divergência de resultado\n> Acusação de emulador\n\nTicket com print + replay.' },
        { ch: '📮・vagas-mediador', t: '📮 VAGAS — MEDIADOR', c: '#9B59B6', d: '**REQ:** 16+, conhecer regras, neutro.\n**SAL:** R$ 0,15/jogador\n**CAND:** Ticket [VAGA MEDIADOR]' },
        { ch: '📮・vaga-influenciador', t: '📮 VAGAS — INFLUENCIADOR', c: '#FF69B4', d: '**REQ:** Canal +500 inscritos.\n**BENEF:** Cargo + coins + divulgação.' }
      ];
      ...
      if (cfgAuto?.auto_post_ranking) {
        try { const ch = f('📊・ranking'); if (ch) panelsToSend.push(ch.send({ embeds: [new EmbedBuilder().setTitle('📊 Ranking Semanal').setColor('#FFD700').setDescription('Sem dados ainda.').setTimestamp()] }).catch(() => {})); } catch {}
      }
      if (cfgAuto?.auto_post_regras) {
        try { const ch = f('📜・regras-analises'); if (ch) panelsToSend.push(ch.send({ embeds: [new EmbedBuilder().setTitle('📜 Regras de Análise').setColor('#00AAFF').setDescription('**1. Provas obrigatórias**\n> Replay, print resultado, print ID\n\n**2. Prazos**\n> 5 min após término\n\n**3. W.O.**\n> Sem entrar em 3 min, sem replay, sem prova\n\n**4. Penalidades**\n> Quebra = entregar round\n> Reincidência = blacklist').setTimestamp()] }).catch(() => {})); } catch {}
      }
      try { const ch = f('・avisos-e-funções'); if (ch) panelsToSend.push(ch.send({ embeds: [new EmbedBuilder().setTitle('📌 Avisos').setColor('#5865F2').setDescription('**Cargos:**\n👑 Owner • DIRETOR • GERENTE\n🛡️ SUPORTE • MEDIADOR\n/👁️ Analista • gg/[org]').setTimestamp()] }).catch(() => {})); } catch {}
      try { const ch = f('⚙️・log-config') || f('🤖・log-filas'); if (ch) panelsToSend.push(ch.send({ embeds: [new EmbedBuilder().setTitle('✅ Setup concluído').setColor('#22c55e').setDescription(`Configurado em **${guild.name}**`).addFields({ name: 'Cargos', value: `${guild.roles.cache.size}`, inline: true }, { name: 'Canais', value: `${guild.channels.cache.size}`, inline: true }).setTimestamp()] }).catch(() => {})); } catch {}

      await Promise.allSettled(panelsToSend);

      // Embeds de aposta — sequencial + ordem MAIOR→MENOR
      try {
        ...
        // 🔧 FIX: sequencial pra garantir ordem
        let totalBet = 0;
...
// ═══════════════════════════════════════════════════════════
// FIM DA PARTE 4/6
// Próxima: PARTE 5/6 — Painéis (Dev/Admin/Loja/FF/Streamer)
// + Comandos + /ajuda
// ═══════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════
// DEV HUB — Reorganizado
// ═══════════════════════════════════════════════════════════
function devHub() {
  const e = new EmbedBuilder()
    .setTitle('👑 Painel Dev')
...
async function devCatServidor() {
  const e = new EmbedBuilder().setTitle('🏗️ Servidor').setColor('#5865F2')
...
async function devCatGerenciamento() {
  const e = new EmbedBuilder().setTitle('🎯 Gerenciamento').setColor('#FFA500')
...
async function devCatModeracao() {
  const e = new EmbedBuilder().setTitle('⚠️ Moderação').setColor('#FF5555')
...
async function devCatSistema() {
  const e = new EmbedBuilder().setTitle('🖥️ Sistema').setColor('#8E44AD')
...
async function devPanelInspector(guildId) {
  ...
  const e1 = new EmbedBuilder().setTitle(`🔍 ${g.name}`).setColor('#5865F2')
    .setDescription(`\`${g.id}\` • <@${g.ownerId}>`)
    .addFields(
...
async function devPanelSimulator() {
  return {
    embeds: [new EmbedBuilder().setTitle('🎬 Simulador').setColor('#9B59B6')
      .setDescription('Roda fluxo completo sem executar de verdade.')],
    components: [new ActionRowBuilder().addComponents(
...
function adminHub() {
  ...
  const menu = new StringSelectMenuBuilder()
    ...
    .addOptions(
      ...
      { label: 'Moderação', description: 'Tickets, usuários', value: 'moderacao', emoji: '⚠️' },
      { label: 'Música', description: 'Player', value: 'musica', emoji: '🎵' },
...
async function admPanelMusica(guild) {
  return {
    embeds: [new EmbedBuilder().setTitle('🎵 Música').setColor('#1DB954').setDescription(await isPremium(guild.id) ? 'Use os controles.' : '💎 Premium')],
    components: [
      new ActionRowBuilder().addComponents(
...
function getCommands() {
  return [
    ...
    new SlashCommandBuilder().setName('painel').setDescription('Painéis').addStringOption(o => o.setName('tipo').setDescription('Tipo').setRequired(true).addChoices(
      { name: 'Ticket', value: 'ticket' }, { name: 'Verificação', value: 'verificacao' }, { name: 'Updates', value: 'updates' }
    ...
    new SlashCommandBuilder().setName('painel_loja').setDescription('Gerenciar painéis da loja')
      .addSubcommand(s => s.setName('abrir').setDescription('Abre painel'))
  ...
  return {
    embeds: [new EmbedBuilder().setTitle('🛡️ Painel Admin').setColor('#ED4245')
      ...
      .addFields(
        ...
        { name: '🎛️ `/painel`', value: 'Posta painéis.\n`ticket`, `verificacao`, `updates`' },
        { name: '🎉 `/sorteio criar`', value: 'Cria sorteio.' },
        { name: '🛒 `/painel_loja`', value: 'Config completa da loja.' },
      )],
    components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('ajuda_back').setLabel('Voltar').setEmoji('↩️').setStyle(ButtonStyle.Secondary))],
  };
}
...
// ═══════════════════════════════════════════════════════════
// FIM DA PARTE 5/6
// Próxima: PARTE 6/6 (FINAL) — Events + interactionCreate
...
client.once('ready', async () => {
  ...
  setInterval(checkDevRoles, 3 * 60 * 1000);
...
client.on('guildCreate', async (g) => {
  ...
  await logImportant('ENTROU', 'Bot adicionado em novo servidor', {
    ...
    fields: [
      ...
      { name: '🚀', value: `${g.premiumSubscriptionCount || 0}`, inline: true },
    ],
...
// ═══════════════════════════════════════════════════════════
// MENSAGENS — anti-spam + comando secreto
// ═══════════════════════════════════════════════════════════
...
client.on('interactionCreate', async (i) => {
  try {
    ...
    // Manutenção
    if (!isDev && i.guild && i.user?.id && !isDeveloper(i.user.id)) {
    ...
    if (i.isChatInputCommand()) {
      ...
      if (c === 'painel') {
        ...
        if (tipo === 'ticket') {
          const panels = await getTicketPanels(guild.id);
          if (!panels.length) return i.reply({ content: '❌ Nenhum painel criado. Use `/admin → Moderação → Tickets`.', flags: EPHEMERAL });
        ...
        if (tipo === 'verificacao') {
          ...
          const b = new ButtonBuilder().setLabel(cfg.verificacao_botao || 'Verificar').setEmoji('✅').setStyle(ButtonStyle.Link).setURL(oauthUrl);
      ...
      if (c === 'painel_loja') {
        ...
        if (sub === 'enviar') {
          ...
          return i.reply({ content: `✅ Em ${ch}`, flags: EPHEMERAL });
        }
        if (sub === 'excluir') {
          ...
          if (p.channel_id && p.message_id) {
            try { const ch = await guild.channels.fetch(p.channel_id).catch(() => null); if (ch) { const m = await ch.messages.fetch(p.message_id).catch(() => null); if (m) await m.delete().catch(() => {}); } } catch {}
          }
    ...
    if (i.isChannelSelectMenu() && i.customId === 'updates_channel_pick') {
      ...
      if (!ch.permissionsFor(guild.members.me)?.has(PermissionFlagsBits.SendMessages)) {
        return i.reply({ content: `❌ Sem permissão em <#${chId}>.`, flags: EPHEMERAL });
      }
    ...
    if (i.isButton()) {
      ...
      if (cid.startsWith('cfg_')) {
        ...
        if (cid === 'cfg_ticket_set_cat') { const m = new ModalBuilder().setCustomId('cfg_ticket_cat_modal').setTitle('Cat'); m.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('cid').setLabel('ID canal').setStyle(TextInputStyle.Short).setRequired(true))); return i.showModal(m); }
      ...
      if (cid.startsWith('cfgset_')) {
        ...
        const c = await getConfig(guild.id);
        if (key === 'ticket_log_channel') { c.ticket_log_channel_id = value; await setConfig(guild.id, c); return i.reply({ content: `✅ Logs em <#${value}>`, flags: EPHEMERAL }); }
        if (key === 'log_channel') { c.log_channel = value; await setConfig(guild.id, c); return i.reply({ content: `✅ Logs em <#${value}>`, flags: EPHEMERAL }); }
        if (key === 'welcome_channel') { c.welcome_channel = value; await setConfig(guild.id, c); return i.reply({ content: `✅ Welcome em <#${value}>`, flags: EPHEMERAL }); }
        if (key === 'suggestion_channel') { c.suggestion_channel = value; await setConfig(guild.id, c); return i.reply({ content: `✅ Sugestões em <#${value}>`, flags: EPHEMERAL }); }
        if (key === 'admin_role') { c.admin_role = value; await setConfig(guild.id, c); return i.reply({ content: `✅ Admin <@&${value}>`, flags: EPHEMERAL }); }
        if (key === 'membro_role') { c.membro_role = value; await setConfig(guild.id, c); return i.reply({ content: `✅ Membro <@&${value}>`, flags: EPHEMERAL }); }
        if (key === 'ticket_cargo') { c.ticket_cargo = value; await setConfig(guild.id, c); return i.reply({ content: `✅ Suporte <@&${value}>`, flags: EPHEMERAL }); }
        if (key === 'autorole_role') { c.autorole_role = value; await setConfig(guild.id, c); return i.reply({ content: `✅ AutoRole <@&${value}>`, flags: EPHEMERAL }); }
      }
      ...
      if (cid.startsWith('adm_') || ns === 'adm') {
        ...
        if (cid === 'adm_paineis') { if (!await isAdmin(i.user, guild)) return; return i.update({ embeds: [new EmbedBuilder().setTitle('🎫 Painéis').setColor('#9B59B6')], components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('adm_p_ticket').setLabel('Ticket').setEmoji('🎫').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('adm_p_verif').setLabel('Verif').setEmoji('✅').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('adm_p_loja').setLabel('Loja').setEmoji('🛒').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('adm_back').setLabel('Voltar').setEmoji('↩️').setStyle(ButtonStyle.Secondary))] }); }
        if (cid === 'adm_p_ticket') { if (!await isAdmin(i.user, guild)) return; const panels = await getTicketPanels(guild.id); if (!panels.length) return i.reply({ content: '❌ Nenhum painel.', flags: EPHEMERAL }); const panel = panels[0]; const msg = await channel.send({ embeds: [buildTicketPanelEmbed(panel)], components: buildTicketPanelComponents(panel) }); await updateTicketPanel(guild.id, panel.id, { canal_id: channel.id, mensagem_id: msg.id }); return i.reply({ content: '✅', flags: EPHEMERAL }); }
        if (cid === 'adm_p_verif') { if (!await isAdmin(i.user, guild)) return; const c = await getConfig(guild.id); const oauthUrl = `https://discord.com/api/oauth2/authorize?client_id=${DISCORD_CLIENT_ID}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&response_type=code&scope=identify%20guilds.join&state=${encodeURIComponent('verify:' + guild.id)}`; const b = new ButtonBuilder().setLabel(c.verificacao_botao).setEmoji('✅').setStyle(ButtonStyle.Link).setURL(oauthUrl); await channel.send({ embeds: [new EmbedBuilder().setColor(c.verificacao_cor).setTitle(c.verificacao_titulo).setDescription(c.verificacao_descricao)], components: [new ActionRowBuilder().addComponents(b)] }); return i.reply({ content: '✅', flags: EPHEMERAL }); }
        ...
        if (cid === 'adm_configurar') { if (!await isAdmin(i.user, guild)) return; return i.update({ embeds: [new EmbedBuilder().setTitle('⚙️ Configurar').setColor('#5865F2')], components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('cfg_canais').setLabel('Canais').setEmoji('📢').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('cfg_cargos').setLabel('Cargos').setEmoji('🎭').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('cfg_ticket').setLabel('Ticket').setEmoji('🎫').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('cfg_moderacao').setLabel('Moderação').setEmoji('🛡️').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('adm_back').setLabel('Voltar').setEmoji('↩️').setStyle(ButtonStyle.Secondary))] }); }
        ...
        if (cid === 'adm_sorteios') { if (!await isAdmin(i.user, guild)) return; return i.update({ embeds: [new EmbedBuilder().setTitle('🎉 Sorteios').setDescription('Use `/sorteio criar`').setColor('#FFD700')], components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('adm_back').setLabel('Voltar').setEmoji('↩️').setStyle(ButtonStyle.Secondary))] }); }
        ...
        if (cid === 'mus_play') { if (!await isAdmin(i.user, guild)) return; const m = new ModalBuilder().setCustomId('modal_mus_play').setTitle('Tocar'); m.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('busca').setLabel('Nome/link').setStyle(TextInputStyle.Short).setRequired(true))); return i.showModal(m); }
        ...
        if (cid === 'mus_vol') { const m = new ModalBuilder().setCustomId('modal_mus_vol').setTitle('Vol'); m.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('v').setLabel('0-200').setStyle(TextInputStyle.Short).setRequired(true))); return i.showModal(m); }
      ...
      if (ns === 'ffm') {
        ...
        if (action === 'confirmar_pag') {
          // 🔧 FIX: se não tem mediador, staff/admin pode confirmar
      ...
      if (ns === 'ffstr') {
        ...
        if (action === 'entrar') {
          ...
          await i.reply({ content: ok ? '✅ Você entrou na lista!\n> Clique em **🔴 Definir Live** pra aparecer ao vivo.' : '✅ Você já estava na lista.', flags: EPHEMERAL });
      ...
      // ===== CUSTOM BET =====
      if (cid === 'custom_bet_preview') { const cfg = await ffGetConfig(guild.id); const fake = { id: 0, format: '1v1 Mobile', value: 5.00, gelo_infinito_players: JSON.stringify([{ userId: i.user.id }]), gelo_normal_players: '[]' }; return i.reply({ content: '🎨 Preview:', embeds: [ffBuildBetEmbed(fake, cfg)], components: [ffBuildBetButtons(0, cfg)], flags: EPHEMERAL }); }
    ...
    if (i.isModalSubmit()) {
      ...
      // Custom bet
      if (cid === 'modal_custom_bet_edit') { if (!await isAdmin(i.user, guild)) return; const cfg = await ffGetConfig(guild.id); const c = cfg?.custom_bet_embed || {}; const u = { title: i.fields.getTextInputValue('title')?.trim() || null, color: normalizeHex(i.fields.getTextInputValue('color')?.trim(), '#f1c40f'), thumbnail: i.fields.getTextInputValue('thumbnail')?.trim() || null, banner: i.fields.getTextInputValue('banner')?.trim() || null, footer: i.fields.getTextInputValue('footer')?.trim() || null }; await ffPatchConfig(guild.id, { custom_bet_embed: { ...c, ...u } }); await logConfig(guild, i.user.id, 'CUSTOM_BET_EDITED', {}); return i.update(await ffPanelCustomEmbed(guild.id)); }
      if (cid === 'modal_custom_bet_buttons') { if (!await isAdmin(i.user, guild)) return; const cfg = await ffGetConfig(guild.id); const c = cfg?.custom_bet_embed || {}; const btns = { gi_label: i.fields.getTextInputValue('gi_label')?.trim() || 'Gelo Infinito', gn_label: i.fields.getTextInputValue('gn_label')?.trim() || 'Gelo Normal', sair_label: i.fields.getTextInputValue('sair_label')?.trim() || 'Sair', gi_emoji: i.fields.getTextInputValue('gi_emoji')?.trim() || '🧊', gn_emoji: i.fields.getTextInputValue('gn_emoji')?.trim() || '🧊' }; await ffPatchConfig(guild.id, { custom_bet_embed: { ...c, buttons: btns } }); await logConfig(guild, i.user.id, 'CUSTOM_BET_BUTTONS', {}); return i.update(await ffPanelCustomEmbed(guild.id)); }
    ...
    // Broadcast scope select (dentro do isStringSelectMenu já processado)
...
// ═══════════════════════════════════════════════════════════
// ROTA: /verify/:guildId — serve HTML (valida user)
// ═══════════════════════════════════════════════════════════
app.get('/verify/:guildId', async (req, res) => {
  ...
  // 🔧 FIX: se não tem user, redireciona pra OAuth
  if (!userId) {
  ...
  const html = buildVerificationHTML(guildId, guildName, guildIcon, userId);
  res.set('Content-Type', 'text/html; charset=utf-8').send(html);
});

// ═══════════════════════════════════════════════════════════
// ROTA: /verify/:guildId/complete — aplica cargo APÓS captcha
// ═══════════════════════════════════════════════════════════
...
app.get('/callback', async (req, res) => {
  ...
  try {
    ...
    // 🔧 FIX: NÃO aplica cargo aqui. Só após resolver o captcha.
    try {
      const g = client.guilds.cache.get(guildId);
    ...
    // Redireciona pro HTML com captcha
...
// ═══════════════════════════════════════════════════════════
// ✅ FIM DO ARQUIVO — 6 PARTES COMPLETAS
// ═══════════════════════════════════════════════════════════
