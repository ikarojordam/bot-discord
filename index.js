...
async function setupServer(guild, type) {
  ...
  if (type === 'loja') {
    ...
    if (ticketChannelLoja) {
      const config = await getConfig(guild.id);
      const ticketEmbed = new EmbedBuilder().setColor('#9B59B6').setTitle(config.ticket_titulo || 'Central de Suporte').setDescription(config.ticket_descricao || 'Clique no botão abaixo para abrir um ticket de suporte.');
