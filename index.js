// ============================================================
// 🤖 FRIOBOT / BOT VENDAS — index.js
// v3.0 — Completo, organizado, otimizado
// ============================================================
...
async function setConfig(gid, cfg) {
  await supabase.from('configs').upsert({ ...cfg, guild_id: gid, updated_at: new Date().toISOString() }, { onConflict: 'guild_id' });
}
...
// ─── Moderação / Manutenção Global ──────────────────────────
async function isMaintenanceMode() {
  const { data } = await supabase.from('maintenance_mode').select('*').eq('id', 1).maybeSingle();
  return !!data?.active;
}
async function setMaintenanceMode(a) {
  await supabase.from('maintenance_mode').upsert({ id: 1, active: a, started_at: a ? new Date().toISOString() : null, updated_at: new Date().toISOString() });
}

// ─── Kill Switch ────────────────────────────────────────────
let KILL_SWITCH_CACHE = { active: false, checked: 0 };
async function isKillSwitchActive() {
  if (Date.now() - KILL_SWITCH_CACHE.checked < 10000) return KILL_SWITCH_CACHE.active;
...
// ═══════════════════════════════════════════════════════════
// 📊 DASHBOARD — coleta de estatísticas 24h/7d
// ═══════════════════════════════════════════════════════════
...
async function sendDevAlert(type, title, description, severity = 'info', metadata = {}) {
  try {
    ...
    await supabase.from('dev_alerts').insert({ type, title, description, severity, metadata });
...
// ═══════════════════════════════════════════════════════════
// 🔍 INSPETOR — deep info de um servidor
// ═══════════════════════════════════════════════════════════
...
// 🕵️ ABUSE TRACKER
// ═══════════════════════════════════════════════════════════
async function trackAbuse(userId, action, guildId = null, limit = 20, windowMs = 60000) {
  try {
    const since = new Date(Date.now() - windowMs).toISOString();
    const { count } = await supabase.from('abuse_tracker')
      .select('*', { count: 'exact', head: true })
      .eq('user_id', userId).eq('action', action).gte('window_start', since);
    if ((count || 0) >= limit) {
      await sendDevAlert('suspicious', 'Abuso detectado', `<@${userId}> executou \`${action}\` **${count}×** em ${windowMs / 1000}s.`, 'warning', { userId, action, count, guildId });
      return true;
    }
    await supabase.from('abuse_tracker').insert({ user_id: userId, guild_id: guildId, action });
    if (Math.random() < 0.05) await supabase.from('abuse_tracker').delete().lt('window_start', new Date(Date.now() - 3600000).toISOString());
    return false;
  } catch { return false; }
}
...
async function setGuildLocale(guildId, locale) {
  await supabase.from('guild_locale').upsert({ guild_id: guildId, locale, updated_at: new Date().toISOString() });
}
...
async function scheduleTempRole(gid, uid, rid, ms) {
  ...
  setTimeout(async () => {
    ...
    await supabase.from('temproles').delete().eq('guild_id', gid).eq('user_id', uid).eq('role_id', rid);
  }, ms);
}
...
async function blockSlashIfMaintenance(i) {
  ...
  // 1️⃣ Manutenção GLOBAL (Dev) — bloqueia TUDO
  if (await isMaintenanceMode() && !isOwnerOrDev) {
    if (['ajuda', 'reportar', 'ping'].includes(i.commandName) && !isOwnerOrDev) {
      // deixa passar comandos essenciais só pra dev — mas se for dev já passou
    }
    if (!isOwnerOrDev) {
  ...
  // 2️⃣ Manutenção ADMIN — bloqueia comandos administrativos
  const cfg = await getConfig(i.guild.id);
  if (cfg.admin_maintenance && !isOwnerOrDev && !(await isAdmin(i.user, i.guild))) {
    const adminCommands = ['admin', 'painel', 'painel_loja', 'enviar_loja', 'sorteio', 'musica', 'call'];
    if (adminCommands.includes(i.commandName)) {
      await i.reply({ content: '🔧 **Manutenção Administrativa ativa.** Comandos de admin temporariamente desabilitados.', flags: EPHEMERAL }).catch(() => {});
      return true;
    }
  }

  // 3️⃣ Manutenção FF — bloqueia /hub apostas
  const ffCfg = await ffGetConfig(i.guild.id);
  if (ffCfg.maintenance && !isOwnerOrDev && !(await isAdmin(i.user, i.guild))) {
    if (i.commandName === 'hub') {
      await i.reply({ content: '🔧 **Manutenção no sistema de apostas ativa.** Voltamos em breve!', flags: EPHEMERAL }).catch(() => {});
      return true;
    }
  }
  return false;
...
async function logMediador(g, uid, action, details = {}) {
  const e = new EmbedBuilder().setTitle('🛡️ Log Mediadores').setColor('#00AAFF')
    .addFields(
      ...
      { name: 'Ação', value: `\`${action}\``, inline: true },
      { name: 'Quando', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true }
    );
...
async function ffPatchPixEmbed(gid, p) {
  await supabase.from('ff_pix_embed').upsert({ guild_id: gid, ...p, updated_at: new Date().toISOString() }, { onConflict: 'guild_id' });
...
function ffThreadName(status, value, ids, matchId) {
  // 💰 Prêmio total = valor × 2 (o que o vencedor leva)
    ...
    if (c?.transcript_channel_id) {
      ...
      if (tch) {
        const em = new EmbedBuilder().setTitle('📝 Transcript Salvo').setColor('#9B59B6')
          .addFields(
            { name: 'Thread', value: ffEscapeHtml(th.name), inline: true },
            { name: 'Match', value: `#${mid || '—'}`, inline: true },
...
// ═══════════════════════════════════════════════════════════
// 🎫 TICKETS — Componentes + Botões Staff
// ═══════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════
// 🏗️ SETUP — ORGANIZAÇÃO (posta tudo automaticamente)
// ═══════════════════════════════════════════════════════════
async function setupOrganizacaoServer(guild, onProgress = null, opts = {}) {
  ...
  try {
    ...
    if (!skipPosting) {
      ...
      // 🎮 POSTAR EMBEDS DE APOSTA (11 canais × N valores)
      try {
...
// ═══════════════════════════════════════════════════════════
// 💰 PAINEL PREMIUM (com ForcePremium)
// ═══════════════════════════════════════════════════════════
...
async function buildPingDetailed() {
  ...
  if (render.ok) {
    ...
    eHost.addFields(
      ...
      { name: '⚡ CPU (últ. 5min)', value: render.cpu != null ? `\`${(render.cpu * 100).toFixed(1)}%\`` : '*sem dados*', inline: true },
      { name: '🧠 RAM (últ. 5min)', value: render.mem != null ? `\`${render.mem.toFixed(0)} MB\`` : '*sem dados*', inline: true }
    );
  } else {
    eHost.setDescription(`❌ Não foi possível obter dados:\n> \`${render.error}\`\n\n**Configure:** \`RENDER_API_KEY\` no .env`);
  }
...
function buildSandboxSnippets() {
  const snippets = [
    ...
    { label: 'Count guilds table', value: "const { count } = await supabase.from('guilds').select('*', { count: 'exact', head: true }); return count" },
...
client.once('ready', async () => {
  ...
  console.log(`🔍 [READY] Iniciando setup pós-login...`);
  ...
  console.log(`🔍 [READY] ✅ Comandos OK. Finalizando ready...`);
  ...
  // 🚨 Alertas automáticos a cada 5min
  setInterval(async () => {
    try {
      ...
      for (const g of client.guilds.cache.values()) {
        if (g.memberCount >= 500) {
          ...
          if (!seen) await sendDevAlert('big_guild', `Servidor grande: ${g.name}`, `Entrou um servidor com **${g.memberCount}** membros!`, 'info', { guild_id: g.id, members: g.memberCount });
        }
      }
  ...
  // 🔄 Auto-Heal automático a cada 5min
  setInterval(async () => {
    try {
...
// ═══════════════════════════════════════════════════════════
// 💬 MESSAGE CREATE — AutoMod + Comandos custom
// ═══════════════════════════════════════════════════════════
...
client.on('interactionCreate', async (i) => {
  try {
    // 🚨 KILL SWITCH — bloqueia TUDO (só /ping pra dev passa)
    const killOn = await isKillSwitchActive();
    if (killOn && i.isRepliable()) {
      const isDev = i.user?.id && isDeveloper(i.user.id);
      const isPingDev = i.isChatInputCommand() && i.commandName === 'ping' && isDev;
      if (!isPingDev) {
        return i.reply({ content: '🚨 **Bot em modo de emergência (Kill Switch).** Tente novamente mais tarde.', flags: EPHEMERAL }).catch(() => {});
      }
    }

    // 🕵️ Detector de abuso
    if (i.user?.id && i.guild) {
      const abusive = await trackAbuse(i.user.id, i.type || 'interaction', i.guild.id, 50, 10000);
      if (abusive) return i.reply({ content: '⚠️ Você está indo muito rápido. Aguarde um pouco.', flags: EPHEMERAL }).catch(() => {});
    }
    ...
    if (i.isChatInputCommand()) {
      ...
      if (c === 'status') {
        if (!isDeveloper(i.user.id)) return;
        const a = i.options.getString('atividade');
        const t = { 'Desenvolvendo': ActivityType.Watching, 'Jogando': ActivityType.Playing };
        ...
        if (sub === 'excluir') {
          ...
          if (p.channel_id && p.message_id) {
            try {
              ...
              if (ch) { const m = await ch.messages.fetch(p.message_id).catch(() => null); if (m) await m.delete().catch(() => {}); }
            } catch {}
          }
    ...
    if (i.isStringSelectMenu()) {
      ...
      if (cid.startsWith('ffm:pick_winner:')) {
        try {
          console.log(`🔍 [WINNER] Iniciando — cid=${cid}`);
          const matchId = cid.split(':')[2];
          const m = await ffGetMatch(matchId);
          console.log(`🔍 [WINNER] Match ${matchId}: ${m ? 'ENCONTRADO' : 'NÃO ENCONTRADO'} • mediator=${m?.mediator_id} • value=${m?.value} • status=${m?.status}`);
          ...
          const isDevChk = isDeveloper(i.user.id);
          const isStaffChk = await isAdmin(i.user, guild);
          if (!isMedChk && !isDevChk && !isStaffChk) return i.reply({ content: '❌ Só o mediador do match pode escolher o vencedor.', flags: EPHEMERAL }).catch(() => {});
          ...
          console.log(`🔍 [WINNER] winner=${winner} • prize=${prize} • fee=${fee} • coins=${coins} • players=${JSON.stringify(players)}`);
          ...
          console.log(`🔍 [WINNER] Match atualizado ✅`);
          ...
          try {
            ...
            console.log(`🔍 [WINNER] Coins creditados ✅`);
          ...
          try {
            if (m.mediator_id) {
              ...
              console.log(`🔍 [WINNER] Taxa mediador ✅`);
            }
          ...
          console.log(`🔍 [WINNER] ✅ Finalizado com sucesso`);
          setTimeout(() => i.channel.setArchived(true).catch(() => {}), 15000);
          return;
        } catch (errW) {
          console.error('❌ [WINNER] Erro completo:', errW);
          try { if (!i.replied && !i.deferred) await i.reply({ content: `❌ Erro ao finalizar: \`${errW.message}\``, flags: EPHEMERAL }); } catch {}
          return;
        }
      }
      ...
      if (cid === 'ticket_pick_type') {
        ...
        if (!t) return i.update({ content: '❌ Tipo inválido.', components: [], embeds: [] });
        await i.deferUpdate();
        try {
          let parentCh = null;
          if (t.channel_id) parentCh = guild.channels.cache.get(t.channel_id) || await guild.channels.fetch(t.channel_id).catch(() => null);
          ...
          console.log(`🔍 [TICKET] Tipo=${typeId} • Canal destino=${parentCh?.name} (${parentCh?.id})`);
          if (!parentCh.permissionsFor(guild.members.me).has(PermissionFlagsBits.CreatePrivateThreads)) {
            return i.followUp({ content: '❌ Bot sem permissão de criar threads privadas nesse canal.', flags: EPHEMERAL });
          }
          const th = await parentCh.threads.create({ name: `${t.emoji || '🎫'}${typeId}-${i.user.username}`.slice(0, 90), autoArchiveDuration: 1440, type: ChannelType.PrivateThread, reason: `Ticket ${t.label} por ${i.user.tag}` });
          ...
          if (cfg.ticket_cargo && cfg.ticket_cargo !== t.role_id) { const r2 = guild.roles.cache.get(cfg.ticket_cargo); if (r2) for (const m of r2.members.values()) await th.members.add(m.id).catch(() => {}); }
          const e = new EmbedBuilder().setColor('#9B59B6').setTitle(`${t.emoji || '🎫'} ${t.label}`)
            .setDescription(`${t.message || 'Aguarde atendimento da equipe.'}\n\n**Aberto por:** <@${i.user.id}>\n**Categoria:** \`${t.label}\`\n**Atendido por:** *aguardando*`)
            .setFooter({ text: `Ticket • ${new Date().toLocaleString('pt-BR')}` }).setTimestamp();
          const pings = [];
          if (t.role_id) pings.push(`<@&${t.role_id}>`);
          else if (cfg.ticket_cargo) pings.push(`<@&${cfg.ticket_cargo}>`);
          ...
          console.log(`🔍 [TICKET] Thread criada: ${th.name} (${th.id})`);
          return i.followUp({ content: `✅ Ticket criado: <#${th.id}>`, flags: EPHEMERAL });
        } catch (err) {
          console.error('❌ [TICKET] Erro ao criar thread:', err);
          return i.followUp({ content: `❌ Erro: \`${err.message}\``, flags: EPHEMERAL });
        }
      }
    ...
    if (i.isButton()) {
      ...
      if (cid.startsWith('adm_') || ns === 'adm') {
        ...
        if (cid === 'adm_maint_toggle') {
          if (!await isAdmin(i.user, guild)) return i.reply({ content: '❌ Só admins.', flags: EPHEMERAL });
          const cfg = await getConfig(guild.id);
          ...
          try {
            ...
            if (ch) await ch.send({ embeds: [new EmbedBuilder().setTitle(nv ? '🔧 MANUTENÇÃO ADMIN ATIVA' : '🟢 ADMIN LIBERADO').setColor(nv ? '#ff5555' : '#22c55e').setDescription(nv ? 'Comandos administrativos temporariamente desabilitados.' : 'Comandos administrativos liberados.').addFields({ name: 'Por', value: `<@${i.user.id}>` }).setTimestamp()] });
          } catch {}
          try { const o = await guild.fetchOwner(); if (o.id !== i.user.id) await o.send(`🔧 Manutenção **Admin** ${nv ? 'ativada' : 'desativada'} em **${guild.name}**.`); } catch {}
          return i.update(await admPanelManutencao(guild));
        }
        if (cid === 'adm_maint_reason') { if (!await isAdmin(i.user, guild)) return i.reply({ content: '❌ Só admins.', flags: EPHEMERAL }); const m = new ModalBuilder().setCustomId('adm_maint_reason_modal').setTitle('Motivo da Manutenção'); m.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('r').setLabel('Motivo').setStyle(TextInputStyle.Paragraph).setRequired(true).setMaxLength(300))); return i.showModal(m); }
      ...
      if (cid.startsWith('dev_')) {
        ...
        // 🎯 DASHBOARD / PING
        if (cid === 'dev_dashboard') {
          const p = await devPanelDashboard();
          if (i.message) return i.update(p).catch(() => {});
          return i.reply({ ...p, flags: EPHEMERAL }).catch(() => {});
        }
        ...
        if (cid === 'dev_alerts_config') {
          ...
          for (let idx = 0; idx < tipos.length; idx++) {
            const t = tipos[idx];
            const enabled = cfgMap[t] !== false;
            row.addComponents(new ButtonBuilder().setCustomId(`dev_alert_toggle:${t}`).setLabel(t).setEmoji(enabled ? '🟢' : '🔴').setStyle(enabled ? ButtonStyle.Success : ButtonStyle.Danger));
        ...
        if (cid.startsWith('dev_alert_toggle:')) {
          const t = cid.split(':')[1];
          const { data } = await supabase.from('dev_alert_config').select('*').eq('type', t).maybeSingle();
          const nv = data ? !data.enabled : false;
          if (data) await supabase.from('dev_alert_config').update({ enabled: nv, updated_at: new Date().toISOString() }).eq('type', t);
          else await supabase.from('dev_alert_config').insert({ type: t, enabled: nv });
          const tipos = ['offline', 'crash', 'big_guild', 'bug_flood', 'mp_fail', 'rate_limit', 'suspicious'];
          const { data: config } = await supabase.from('dev_alert_config').select('*');
          const cfgMap = {}; for (const c of config || []) cfgMap[c.type] = c.enabled;
          const rows = []; let row = new ActionRowBuilder();
        ...
        if (cid.startsWith('dev_inspector_backup:')) { const gid = cid.split(':')[1]; const g = client.guilds.cache.get(gid); if (!g) return i.reply({ content: '❌', flags: EPHEMERAL }); const data = { name: g.name, icon: g.iconURL(), roles: g.roles.cache.map(r => ({ name: r.name, color: r.hexColor })), channels: g.channels.cache.map(c => ({ name: c.name, type: c.type })) }; await supabase.from('guild_backups').insert({ guild_id: gid, data }); await logDevAction(i.user.id, 'inspector_backup', gid, {}); return i.reply({ content: `✅ Backup de **${g.name}** salvo.`, flags: EPHEMERAL }); }
        if (cid.startsWith('dev_inspector_notes:')) { const gid = cid.split(':')[1]; return i.update(await devPanelNotes(gid)); }
        if (cid.startsWith('dev_inspector_leave:')) { const gid = cid.split(':')[1]; const g = client.guilds.cache.get(gid); if (!g) return i.reply({ content: '❌ Já saiu.', flags: EPHEMERAL }); await logDevAction(i.user.id, 'inspector_leave', gid, {}); await i.reply({ content: `🚪 Saindo de **${g.name}**...`, flags: EPHEMERAL }); setTimeout(() => g.leave().catch(() => {}), 2000); return; }
        ...
        if (cid.startsWith('dev_staff_bl_add:')) {
          ...
          if (ex) { await supabase.from('staff_blacklist').delete().eq('user_id', uid); return i.reply({ content: `✅ <@${uid}> removido da blacklist de staff.`, flags: EPHEMERAL }); }
        ...
        if (cid === 'dev_dead_cleanup') { const dead = await getDeadServers(); const e = new EmbedBuilder().setTitle('⚠️ Confirmar limpeza').setColor('#ff5555').setDescription(`Sair de **${dead.length}** servidores mortos?\n\nEssa ação **NÃO** pode ser desfeita.`); return i.reply({ embeds: [e], components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('dev_dead_cleanup_confirm').setLabel(`Sim, sair de ${dead.length}`).setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId('dev_dead_servers').setLabel('Cancelar').setStyle(ButtonStyle.Secondary))], flags: EPHEMERAL }); }
        if (cid === 'dev_dead_cleanup_confirm') { await i.reply({ content: '🧹 Limpando...', flags: EPHEMERAL }); const dead = await getDeadServers(); let ok = 0; for (const s of dead) { const g = client.guilds.cache.get(s.guild_id); if (g) { await g.leave().catch(() => {}); ok++; await sleep(500); } } await logDevAction(i.user.id, 'dead_cleanup', null, { count: ok }); return i.editReply({ content: `✅ Saí de **${ok}** servidores mortos.` }); }

        // 🌐 EVENTOS
        if (cid === 'dev_global_events') return i.update(await devPanelGlobalEvents());
        if (cid === 'dev_event_coins_double') { const m = new ModalBuilder().setCustomId('modal_event_coins_double').setTitle('Dobro de Coins'); m.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('title').setLabel('Título').setStyle(TextInputStyle.Short).setValue('Dobro de Coins').setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('hours').setLabel('Duração (horas, 0=perm)').setStyle(TextInputStyle.Short).setValue('24').setRequired(true))); return i.showModal(m); }
        if (cid === 'dev_event_no_fee') { const m = new ModalBuilder().setCustomId('modal_event_no_fee').setTitle('Sem Taxa'); m.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('title').setLabel('Título').setStyle(TextInputStyle.Short).setValue('Sem Taxa!').setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('hours').setLabel('Duração (horas)').setStyle(TextInputStyle.Short).setValue('24').setRequired(true))); return i.showModal(m); }
        if (cid === 'dev_event_bonus') { const m = new ModalBuilder().setCustomId('modal_event_bonus').setTitle('Bônus Coins'); m.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('title').setLabel('Título').setStyle(TextInputStyle.Short).setValue('Bônus').setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('multiplier').setLabel('Multiplier').setStyle(TextInputStyle.Short).setValue('2').setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('hours').setLabel('Duração (horas)').setStyle(TextInputStyle.Short).setValue('24').setRequired(true))); return i.showModal(m); }
        if (cid === 'dev_event_sorteio') { const m = new ModalBuilder().setCustomId('modal_event_sorteio').setTitle('Sorteio'); m.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('prize').setLabel('Prêmio em coins').setStyle(TextInputStyle.Short).setValue('500').setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('winners').setLabel('Ganhadores').setStyle(TextInputStyle.Short).setValue('5').setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('hours').setLabel('Duração (horas)').setStyle(TextInputStyle.Short).setValue('24').setRequired(true))); return i.showModal(m); }
        if (cid === 'dev_event_stop_all') { await supabase.from('dev_global_events').update({ active: false }).eq('active', true); await logDevAction(i.user.id, 'stop_all_events', null, {}); return i.reply({ content: '🛑 Todos os eventos globais foram parados.', flags: EPHEMERAL }); }
        if (cid === 'dev_event_notify') { const events = await getActiveGlobalEvents(); if (!events.length) return i.reply({ content: '❌ Nenhum evento ativo.', flags: EPHEMERAL }); const r = await enviarAvisoGlobal('🎉 Eventos Globais Ativos!', events.map(ev => `**${ev.title}** — ${ev.multiplier}×`).join('\n')); return i.reply({ content: `📢 Notificado em ${r.canaisOk} canais e ${r.dmsOk} DMs.`, flags: EPHEMERAL }); }
        ...
        if (cid === 'dev_kill_reason') { const m = new ModalBuilder().setCustomId('modal_kill_reason').setTitle('Motivo do Kill Switch'); m.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('reason').setLabel('Motivo').setStyle(TextInputStyle.Paragraph).setRequired(true).setMaxLength(300))); return i.showModal(m); }
        ...
        if (cid === 'dev_rejoin_all') { await checkAutoRejoin(); return i.reply({ content: '🚀 Tentativa executada. Verifique em segundos.', flags: EPHEMERAL }); }
        if (cid === 'dev_rejoin_manual') { const m = new ModalBuilder().setCustomId('modal_rejoin_manual').setTitle('Rejoin'); m.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('invite').setLabel('Link do convite').setStyle(TextInputStyle.Short).setRequired(true))); return i.showModal(m); }
        ...
        if (cid === 'dev_simulate_run') {
          ...
          try {
            ...
            const e = new EmbedBuilder().setTitle('🎬 Resultado').setColor(r.errCount === 0 ? '#22c55e' : r.errCount < 3 ? '#FFA500' : '#FF5555')
              ...
              .addFields({ name: '✅ Sucesso', value: `\`${r.okCount}\``, inline: true }, { name: '❌ Falhas', value: `\`${r.errCount}\``, inline: true }, { name: '⏱️ Total', value: `\`${r.totalMs}ms\``, inline: true }).setTimestamp();
            return i.editReply({ embeds: [e] });
          } catch (err) { return i.editReply({ content: `❌ ${err.message}` }); }
        }

        // 🔄 AUTO-HEAL
        if (cid === 'dev_autoheal') { await i.deferReply({ flags: EPHEMERAL }); const r = await runAutoHeal(); return i.editReply({ content: `🔄 Auto-heal executado:\n> 🧵 Threads travadas canceladas: **${r.canceledThreads}**\n> ⚠️ Matches alertados: **${r.alertedMatches}**\n> 💳 PIX travados cancelados: **${r.canceledPix}**` }); }

        // 🌐 LOCALE
        if (cid === 'dev_locale') return i.reply({ ...(await devPanelLocale(guild)), flags: EPHEMERAL });
        if (cid === 'dev_locale_pt') { await setGuildLocale(guild.id, 'pt-BR'); return i.reply({ content: '✅ Idioma: 🇧🇷 Português', flags: EPHEMERAL }); }
        if (cid === 'dev_locale_en') { await setGuildLocale(guild.id, 'en-US'); return i.reply({ content: '✅ Language: 🇺🇸 English', flags: EPHEMERAL }); }
        if (cid === 'dev_locale_es') { await setGuildLocale(guild.id, 'es-ES'); return i.reply({ content: '✅ Idioma: 🇪🇸 Español', flags: EPHEMERAL }); }
        ...
        if (cid === 'dev_forcepremium_user') { const m = new ModalBuilder().setCustomId('modal_forcepremium_user').setTitle('ForcePremium — Usuário'); m.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('user_id').setLabel('ID do usuário').setStyle(TextInputStyle.Short).setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('guild_id').setLabel('ID do servidor (opcional)').setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('days').setLabel('Dias (0=perm)').setStyle(TextInputStyle.Short).setValue('0').setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('reason').setLabel('Motivo').setStyle(TextInputStyle.Paragraph).setRequired(false).setMaxLength(200))); return i.showModal(m); }
        if (cid === 'dev_forcepremium_list') { const { data } = await supabase.from('force_premium').select('*').order('granted_at', { ascending: false }).limit(30); if (!data?.length) return i.reply({ content: '📋 Nenhum ForcePremium ativo.', flags: EPHEMERAL }); const lines = data.map(f => { const exp = f.permanent ? '♾️ permanente' : (f.expires_at ? `<t:${Math.floor(new Date(f.expires_at).getTime() / 1000)}:R>` : '?'); return `**${f.scope === 'guild' ? '🌐 Servidor' : '👤 User'}** \`${f.target_id}\`\n> ${exp} • por <@${f.granted_by}>${f.reason ? ` • *${f.reason}*` : ''}`; }); return i.reply({ embeds: [new EmbedBuilder().setTitle('🎯 ForcePremium — Ativos').setColor('#FFD700').setDescription(lines.join('\n\n').substring(0, 4000))], flags: EPHEMERAL }); }
        if (cid === 'dev_forcepremium_clear') { const { count } = await supabase.from('force_premium').select('*', { count: 'exact', head: true }); if (!count) return i.reply({ content: '📋 Nada pra limpar.', flags: EPHEMERAL }); const e = new EmbedBuilder().setTitle('⚠️ Confirmar').setColor('#ff5555').setDescription(`Apagar **${count}** registros?`); return i.reply({ embeds: [e], components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('dev_forcepremium_clear_confirm').setLabel('Sim').setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId('dev_back').setLabel('Cancelar').setStyle(ButtonStyle.Secondary))], flags: EPHEMERAL }); }
        ...
        if (cid === 'dev_maint_notify') { await i.reply({ content: '📢 Enviando...', flags: EPHEMERAL }); const gOn = await isMaintenanceMode(); const r = await enviarAvisoGlobal(gOn ? '🔧 Manutenção Global Ativa' : '✅ Sistemas Restaurados', gOn ? 'O bot está em manutenção global. Voltamos em breve!' : 'Os sistemas foram restaurados. Tudo funcionando novamente!'); return i.editReply({ content: `✅ ${r.canaisOk} canais • ${r.dmsOk} DMs` }); }
      ...
      if (ns === 'ffm') {
        ...
        if (action === 'chamar_analista') {
          ...
          if (!isMedChk && !isOlhChk && !isStaffChk && !isOwnerChk) return i.reply({ content: '❌ Só mediadores/staff podem chamar analista.', flags: EPHEMERAL });
      ...
      if (ns === 'ffana') {
        try {
          ...
          const hasOlh = cfg.olhinho_role_id && i.member.roles.cache.has(cfg.olhinho_role_id);
          console.log(`🔍 [ANALISTA] ${i.user.tag}: hasAna=${hasAna} hasOlh=${hasOlh} isO=${isO} isS=${isS}`);
          if (!hasAna && !hasOlh && !isO && !isS) return i.reply({ content: '❌ Só analistas/olhinho podem entrar.', flags: EPHEMERAL });
          ...
          console.error('❌ [ANALISTA] Erro:', errAna);
      ...
      if (cid.startsWith('ticket_assumir:')) {
        const th = i.channel;
        if (!th?.isThread()) return i.reply({ content: '❌ Só funciona em ticket (thread).', flags: EPHEMERAL });
        const okStaff = await isTicketStaff(i.user, guild);
        if (!okStaff) return i.reply({ content: '❌ Apenas **staff/suporte** pode assumir tickets.', flags: EPHEMERAL });
        try {
          const { data: td } = await supabase.from('ticket_data').select('*').eq('thread_id', th.id).maybeSingle();
          if (td?.assumed_by) return i.reply({ content: `⚠️ Esse ticket já foi assumido por <@${td.assumed_by}>.`, flags: EPHEMERAL });
          ...
          } catch (e) { console.error('Erro edit embed assumir:', e.message); }
          await th.send({ content: `🙋 <@${i.user.id}> assumiu este ticket. Bom atendimento!` });
          return i.reply({ content: '✅ Você assumiu o ticket!', flags: EPHEMERAL });
        } catch (err) { console.error('❌ [TICKET ASSUMIR]', err); return i.reply({ content: `❌ Erro: \`${err.message}\``, flags: EPHEMERAL }); }
      }
      if (cid.startsWith('ticket_priority:')) {
        const th = i.channel;
        if (!th?.isThread()) return i.reply({ content: '❌ Só funciona em ticket (thread).', flags: EPHEMERAL });
        const okStaff = await isTicketStaff(i.user, guild);
        if (!okStaff) return i.reply({ content: '❌ Apenas **staff/suporte** pode marcar prioridade.', flags: EPHEMERAL });
        try {
          const { data: td } = await supabase.from('ticket_data').select('*').eq('thread_id', th.id).maybeSingle();
          ...
          const newName = newPriority ? `🔴 ${baseName}`.slice(0, 100) : baseName;
          await th.setName(newName).catch(() => {});
          try {
            const msgs = await th.messages.fetch({ limit: 30 });
            const mainMsg = msgs.find(m => m.author.id === client.user.id && m.embeds[0]?.title && m.components?.length);
            if (mainMsg) {
          ...
          } catch (e) { console.error('Erro edit embed prioridade:', e.message); }
          await th.send({ content: newPriority ? `🔴 **PRIORIDADE ALTA** definida por <@${i.user.id}>. Atendimento urgente!` : `⚪ Prioridade removida por <@${i.user.id}>.` });
          return i.reply({ content: newPriority ? '🔴 Prioridade ATIVADA' : '⚪ Prioridade desativada', flags: EPHEMERAL });
        } catch (err) { console.error('❌ [TICKET PRIORIDADE]', err); return i.reply({ content: `❌ Erro: \`${err.message}\``, flags: EPHEMERAL }); }
      }
      ...
      if (cid.startsWith('ticket_open:')) {
        ...
        const types = parseJson(cfg.ticket_types, []);
        const t = types.find(x => x.id === typeId);
        if (!t) return i.reply({ content: '❌ Tipo inválido.', flags: EPHEMERAL });
        await i.deferReply({ flags: EPHEMERAL });
        try {
          let parentCh = null;
          if (t.channel_id) parentCh = guild.channels.cache.get(t.channel_id) || await guild.channels.fetch(t.channel_id).catch(() => null);
          if (!parentCh && cfg.ticket_category_id) parentCh = guild.channels.cache.get(cfg.ticket_category_id) || await guild.channels.fetch(cfg.ticket_category_id).catch(() => null);
          if (!parentCh) parentCh = i.channel;
          console.log(`🔍 [TICKET-OPEN] Tipo=${typeId} • Canal=${parentCh?.name} (${parentCh?.id})`);
          ...
          const th = await parentCh.threads.create({ name: `${t.emoji || '🎫'}${typeId}-${i.user.username}`.slice(0, 90), autoArchiveDuration: 1440, type: ChannelType.PrivateThread, reason: `Ticket ${t.label}` });
          await th.members.add(i.user.id).catch(() => {});
          if (t.role_id) { const r = guild.roles.cache.get(t.role_id); if (r) for (const m of r.members.values()) await th.members.add(m.id).catch(() => {}); }
          if (cfg.ticket_cargo && cfg.ticket_cargo !== t.role_id) { const r2 = guild.roles.cache.get(cfg.ticket_cargo); if (r2) for (const m of r2.members.values()) await th.members.add(m.id).catch(() => {}); }
          const e = new EmbedBuilder().setColor('#9B59B6').setTitle(`${t.emoji || '🎫'} ${t.label}`)
            .setDescription(`${t.message || 'Aguarde.'}\n\n**Aberto por:** <@${i.user.id}>\n**Atendido por:** *aguardando*`)
            .setFooter({ text: `Ticket • ${new Date().toLocaleString('pt-BR')}` }).setTimestamp();
          const pings = [];
          if (t.role_id) pings.push(`<@&${t.role_id}>`);
          else if (cfg.ticket_cargo) pings.push(`<@&${cfg.ticket_cargo}>`);
        ...
        } catch (err) { console.error('❌ [TICKET-OPEN] Erro:', err); return i.editReply({ content: `❌ Erro: \`${err.message}\`` }); }
      }
    ...
    if (i.isModalSubmit()) {
      ...
      // ─── ADM ───────────────────────────────────────────────
      if (cid === 'adm_maint_reason_modal') { if (!await isAdmin(i.user, guild)) return i.reply({ content: '❌ Só admins.', flags: EPHEMERAL }); const r = i.fields.getTextInputValue('r').trim(); const cfg = await getConfig(guild.id); await setConfig(guild.id, { ...cfg, admin_maintenance_reason: r }); return i.reply({ content: '✅ Motivo salvo.', flags: EPHEMERAL }); }
      ...
      if (cid === 'modal_kill_reason') { if (!isDeveloper(i.user.id)) return; const reason = i.fields.getTextInputValue('reason').trim(); await supabase.from('kill_switch').update({ reason }).eq('id', 1); return i.reply({ content: `✅ Motivo salvo: ${reason}`, flags: EPHEMERAL }); }
      if (cid === 'modal_preview_create') {
        if (!isDeveloper(i.user.id)) return;
        ...
        return i.reply({ content: '🎨 **Preview (só você vê):**', embeds: [e], flags: EPHEMERAL });
      }
      if (cid === 'modal_rejoin_manual') { if (!isDeveloper(i.user.id)) return; const link = i.fields.getTextInputValue('invite').trim(); const code = link.split('/').pop(); const inv = await client.fetchInvite(code).catch(() => null); if (!inv) return i.reply({ content: '❌ Convite inválido.', flags: EPHEMERAL }); try { const g = await inv.accept(); return i.reply({ content: `✅ Entrei em **${g.name}**!`, flags: EPHEMERAL }); } catch (e) { return i.reply({ content: `❌ ${e.message}`, flags: EPHEMERAL }); } }
      if (cid === 'modal_inspector') { if (!isDeveloper(i.user.id)) return; const gid = i.fields.getTextInputValue('guild_id').trim(); return i.reply({ ...(await devPanelInspector(gid)), flags: EPHEMERAL }); }
      if (cid.startsWith('modal_staff_bl_add:')) { if (!isDeveloper(i.user.id)) return; const uid = cid.split(':')[1]; const reason = i.fields.getTextInputValue('reason').trim(); await supabase.from('staff_blacklist').upsert({ user_id: uid, reason, added_by: i.user.id }); await logDevAction(i.user.id, 'staff_blacklist_add', null, { uid, reason }); await supabase.from('ff_mediator_queue').delete().eq('user_id', uid); await supabase.from('ff_analyst_queue').delete().eq('user_id', uid); return i.reply({ content: `🚫 <@${uid}> banido de ser staff em todos os servidores.\n> Motivo: ${reason}`, flags: EPHEMERAL }); }
      if (cid === 'modal_event_coins_double') { if (!isDeveloper(i.user.id)) return; const title = i.fields.getTextInputValue('title').trim(); const hours = parseInt(i.fields.getTextInputValue('hours')) || 24; await createGlobalEvent('coins_double', title, 2, hours, i.user.id); return i.reply({ content: `✅ Evento **${title}** criado (2× coins) por ${hours}h.`, flags: EPHEMERAL }); }
      if (cid === 'modal_event_no_fee') { if (!isDeveloper(i.user.id)) return; const title = i.fields.getTextInputValue('title').trim(); const hours = parseInt(i.fields.getTextInputValue('hours')) || 24; await createGlobalEvent('no_fee', title, 0, hours, i.user.id); return i.reply({ content: `✅ Evento **${title}** criado (sem taxa) por ${hours}h.`, flags: EPHEMERAL }); }
      if (cid === 'modal_event_bonus') { if (!isDeveloper(i.user.id)) return; const title = i.fields.getTextInputValue('title').trim(); const multiplier = parseFloat(i.fields.getTextInputValue('multiplier')) || 2; const hours = parseInt(i.fields.getTextInputValue('hours')) || 24; await createGlobalEvent('aposta_bonus', title, multiplier, hours, i.user.id); return i.reply({ content: `✅ Evento **${title}** criado (${multiplier}× bônus) por ${hours}h.`, flags: EPHEMERAL }); }
      if (cid === 'modal_event_sorteio') { if (!isDeveloper(i.user.id)) return; const prize = parseInt(i.fields.getTextInputValue('prize')) || 500; const winners = parseInt(i.fields.getTextInputValue('winners')) || 5; const hours = parseInt(i.fields.getTextInputValue('hours')) || 24; await createGlobalEvent('sorteio', `Sorteio ${prize} coins`, prize, hours, i.user.id); const r = await enviarAvisoGlobal(`🎉 Sorteio Global de ${prize} coins!`, `Concorra a **${prize} coins**! Serão **${winners} ganhadores**. Termina em ${hours}h!`); return i.reply({ content: `✅ Sorteio criado e notificado em ${r.canaisOk} canais.`, flags: EPHEMERAL }); }
      if (cid.startsWith('modal_note_add:')) { if (!isDeveloper(i.user.id)) return; const gid = cid.split(':')[1]; const note = i.fields.getTextInputValue('note').trim(); await addGuildNote(gid, note, i.user.id); return i.reply({ content: '✅ Nota adicionada.', flags: EPHEMERAL }); }
      if (cid === 'modal_sandbox') {
        if (!isDeveloper(i.user.id)) return i.reply({ content: '❌ Só devs.', flags: EPHEMERAL });
        ...
        try {
          const fn = new Function('client', 'guild', 'member', 'channel', 'EmbedBuilder', 'ActionRowBuilder', 'ButtonBuilder', 'ButtonStyle', 'supabase', 'sleep', 'logError', 'console',
            `return (async () => { ${code} })();`);
      ...
      if (cid === 'dev_sandbox_snippet_pick') {
        ...
        m.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('code').setLabel('Código JS').setStyle(TextInputStyle.Paragraph).setValue(i.values[0]).setRequired(true)));
        return i.showModal(m);
      }
      if (cid === 'modal_forcepremium_guild') {
        ...
        if (!targetGuild) return i.reply({ content: `❌ Bot não está no servidor \`${guildId}\`.`, flags: EPHEMERAL });
        ...
        try { const owner = await targetGuild.fetchOwner(); if (owner && owner.id !== i.user.id) await owner.send(`💎 **Premium ativado** no seu servidor!\n${permanent ? '> ♾️ Permanente' : `> ⏳ ${days} dias`}${reason ? `\n> 📝 Motivo: ${reason}` : ''}`); } catch {}
        return i.reply({ content: `✅ ForcePremium aplicado em **${targetGuild.name}** (${permanent ? '♾️' : `${days}d`}).`, flags: EPHEMERAL });
      }
      if (cid === 'modal_forcepremium_user') {
        ...
        try { await target.send(`💎 **Premium concedido** a você por um desenvolvedor!\n${guildId ? `> 🌐 Servidor: \`${guildId}\`\n` : '> 🌐 Global\n'}${permanent ? '> ♾️ Permanente' : `> ⏳ ${days} dias`}${reason ? `\n> 📝 Motivo: ${reason}` : ''}`).catch(() => {}); } catch {}
        return i.reply({ content: `✅ ForcePremium aplicado em **${target.tag}**${guildId ? ` no servidor \`${guildId}\`` : ' (global)'}.`, flags: EPHEMERAL });
      }
      if (cid === 'modal_inject_coins') { if (!isDeveloper(i.user.id)) return; const gid = i.fields.getTextInputValue('guild_id').trim(); const uid = i.fields.getTextInputValue('user_id').trim(); const amt = parseInt(i.fields.getTextInputValue('amount')) || 0; const reason = i.fields.getTextInputValue('reason').trim(); const g = client.guilds.cache.get(gid); if (!g) return i.reply({ content: `❌ Bot não está no servidor.`, flags: EPHEMERAL }); const { data: p } = await supabase.from('ff_players').select('coins').eq('guild_id', gid).eq('user_id', uid).maybeSingle(); if (p) await supabase.from('ff_players').update({ coins: Number(p.coins || 0) + amt }).eq('guild_id', gid).eq('user_id', uid); else await supabase.from('ff_players').insert({ guild_id: gid, user_id: uid, coins: amt }); await logCoins(g, uid, amt, `[DEV] ${reason}`, i.user.id); await logDevAction(i.user.id, 'inject_coins', gid, { uid, amt, reason }); try { const u = await client.users.fetch(uid); await u.send(`💰 Você recebeu **${amt} coins** em **${g.name}**!\n> Motivo: ${reason}`); } catch {} return i.reply({ content: `✅ Injetado **${amt} coins** em <@${uid}>.`, flags: EPHEMERAL }); }
      if (cid === 'modal_inject_product') { if (!isDeveloper(i.user.id)) return; const gid = i.fields.getTextInputValue('guild_id').trim(); const uid = i.fields.getTextInputValue('user_id').trim(); const pid = i.fields.getTextInputValue('product_id').trim(); const reason = i.fields.getTextInputValue('reason').trim(); const g = client.guilds.cache.get(gid); if (!g) return i.reply({ content: `❌ Bot não está no servidor.`, flags: EPHEMERAL }); const { data: prod } = await supabase.from('products').select('*').eq('id', pid).maybeSingle(); if (!prod) return i.reply({ content: `❌ Produto não encontrado.`, flags: EPHEMERAL }); const { data: o } = await supabase.from('orders').insert({ guild_id: gid, user_id: uid, status: 'delivered', subtotal: prod.price, total: prod.price, paid_at: new Date().toISOString() }).select().single(); await supabase.from('order_items').insert({ order_id: o.id, product_id: prod.id, product_name: prod.name, quantity: 1, unit_price: prod.price, total: prod.price }); await logDevAction(i.user.id, 'inject_product', gid, { uid, pid, reason }); try { const u = await client.users.fetch(uid); await u.send(`🎁 Você recebeu **${prod.name}** em **${g.name}**!\n> Motivo: ${reason}`); } catch {} return i.reply({ content: `✅ Produto injetado.`, flags: EPHEMERAL }); }
      if (cid === 'modal_inject_role') { if (!isDeveloper(i.user.id)) return; const gid = i.fields.getTextInputValue('guild_id').trim(); const uid = i.fields.getTextInputValue('user_id').trim(); const rid = i.fields.getTextInputValue('role_id').trim(); const reason = i.fields.getTextInputValue('reason').trim(); const g = client.guilds.cache.get(gid); if (!g) return i.reply({ content: `❌ Bot não está no servidor.`, flags: EPHEMERAL }); const m = await g.members.fetch(uid).catch(() => null); if (!m) return i.reply({ content: `❌ Usuário não está no servidor.`, flags: EPHEMERAL }); const role = g.roles.cache.get(rid); if (!role) return i.reply({ content: `❌ Cargo não existe.`, flags: EPHEMERAL }); try { await m.roles.add(role, `[DEV] ${reason}`); } catch (e) { return i.reply({ content: `❌ ${e.message}`, flags: EPHEMERAL }); } await logDevAction(i.user.id, 'inject_role', gid, { uid, rid, reason }); return i.reply({ content: `✅ Cargo **${role.name}** dado a <@${uid}>.`, flags: EPHEMERAL }); }
      if (cid === 'modal_inject_premium') { if (!isDeveloper(i.user.id)) return; const gid = i.fields.getTextInputValue('guild_id').trim(); const days = parseInt(i.fields.getTextInputValue('days')) || 30; const reason = i.fields.getTextInputValue('reason').trim(); const g = client.guilds.cache.get(gid); if (!g) return i.reply({ content: `❌ Bot não está no servidor.`, flags: EPHEMERAL }); const permanent = days === 0; const expiresAt = permanent ? null : new Date(Date.now() + days * 86400000).toISOString(); await supabase.from('force_premium').upsert({ scope: 'guild', target_id: gid, permanent, expires_at: expiresAt, reason, granted_by: i.user.id, granted_at: new Date().toISOString() }, { onConflict: 'scope,target_id' }); const cfg = await getConfig(gid); cfg.is_premium = true; cfg.premium_expires_at = expiresAt; await setConfig(gid, cfg); await logDevAction(i.user.id, 'inject_premium', gid, { days, reason }); try { const owner = await g.fetchOwner(); if (owner && owner.id !== i.user.id) await owner.send(`💎 **Premium ativado**!\n> ${permanent ? 'Permanente' : `${days} dias`}\n> Motivo: ${reason}`); } catch {} return i.reply({ content: `✅ Premium injetado em **${g.name}**.`, flags: EPHEMERAL }); }
    }
