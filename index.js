const { Client, GatewayIntentBits, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, ModalBuilder, TextInputBuilder, TextInputStyle, SlashCommandBuilder, PermissionFlagsBits, ChannelType, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, AttachmentBuilder, AuditLogEvent } = require('discord.js');
...
const REDIRECT_URI = process.env.REDIRECT_URI || `https://${process.env.RENDER_EXTERNAL_HOSTNAME}/callback`; // Render fornece hostname
...
// ========== PROTEÇÃO ANTI-CÓPIA (simples) ==========
// Verifica se o código foi adulterado comparando um hash hardcoded
const EXPECTED_HASH = 'coloque-um-hash-do-seu-codigo-aqui'; // substitua pelo hash real do seu index.js
const codeHash = crypto.createHash('sha256').update(process.env.DISCORD_TOKEN || '').digest('hex');
// Nota: essa verificação é básica e pode ser contornada; serve apenas como dissuasão.
