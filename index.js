...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (commandName === 'levar_membros') {
      ...
      for (const row of data) {
        // Se já estiver no servidor, pula
    ...
    if (commandName === 'listar_verificados') {
      ...
      // Conta quantos estão expirados (refresh token pode estar inválido)
      const agora = Date.now();
      const validos = data.filter(v => v.expires_at && new Date(v.expires_at).getTime() > 0).length;

      // Amostra dos primeiros 15 IDs
