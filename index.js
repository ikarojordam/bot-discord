// ============================================================
// 🤖 FRIOBOT — index.js
// v5.0 — FIX: removidos timeouts agressivos, atalhos silenciosos e abuse limit
// ============================================================
...
// ─── ✅ FIX: getConfig SEM withTimeout, com catch seguro ────
...
// ─── ✅ FIX: getSettings SEM withTimeout ────────────────────
...
// ─── Manutenção Global (COM CACHE, sem withTimeout) ────────
...
// ─── Kill Switch (COM CACHE, sem withTimeout) ──────────────
...
// ─── ✅ FIX: trackAbuse com limite ALTO (200 em 10s) e cache separado ───
...
// ─── Setups (Loja, Comunidade, Organização, Apostas) ────────
async function setupLojaServer(guild, onProgress = null) {
  ...
  try {
    ...
    const panelsToSend = [];
    try { const ch = created['📩・suporte']; if (ch) { const cfgT = await getConfig(guild.id); const ticketTypes = [{ id: 'suporte', label: 'Suporte Geral', emoji: '🛠️', channel_id: ch.id, role_id: roles['T1cket']?.id || null, message: 'Descreva seu problema.' }, { id: 'compra', label: 'Comprar Produto', emoji: '🛒', channel_id: ch.id, role_id: roles['V2ndas']?.id || null, message: 'Descreva o produto.' }, { id: 'reembolso', label: 'Reembolso', emoji: '💸', channel_id: ch.id, role_id: roles['CEO']?.id || null, message: 'Explique o motivo.' }]; await setConfig(guild.id, { ...(await getConfig(guild.id)), ticket_types: ticketTypes, ticket_category_id: ch.id }); const e = new EmbedBuilder().setColor('#9B59B6').setTitle(cfgT.ticket_titulo).setDescription(cfgT.ticket_descricao + '\n\n**Tipos:**\n🛠️ Suporte\n🛒 Compra\n💸 Reembolso'); panelsToSend.push(ch.send({ embeds: [e], components: await buildTicketComponents(guild, cfgT) }).catch(() => {})); } } catch {}
    try { const ch = created['✅・verificação']; if (ch) { const url = `https://discord.com/api/oauth2/authorize?client_id=${DISCORD_CLIENT_ID}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&response_type=code&scope=identify%20guilds.join&state=${guild.id}`; const e = new EmbedBuilder().setColor('#00FF00').setTitle('✅ Verificação').setDescription('Clique abaixo para se verificar.'); const b = new ButtonBuilder().setLabel('Verificar').setEmoji('✅').setStyle(ButtonStyle.Link).setURL(url); panelsToSend.push(ch.send({ embeds: [e], components: [new ActionRowBuilder().addComponents(b)] }).catch(() => {})); } } catch {}
    for (const [channelName, category] of [['🛒・n1tradas', 'D1SCORD'], ['🛒・l1nk', 'D1SCORD'], ['🛒・impuls0s', 'D1SCORD'], ['🛒・at1vações', 'D1SCORD'], ['⭐・g1ft', 'D1SCORD'], ['🛒・pix-infinit9', 'VARIEDADES'], ['🛒・m1necraft', 'VARIEDADES'], ['🛒・r0bux', 'VARIEDADES'], ['⭐・str3amings', 'VARIEDADES']]) { const ch = created[channelName]; if (!ch) continue; try { const s = await getSettings(guild.id); const friendly = channelName.replace(/^[^a-z0-9A-Z]+/, '').replace(/・/g, ' · ').replace(/[_-]/g, ' ').trim(); const panel = await createShopPanel(guild.id, { name: friendly || s.store_name || 'Loja', description: s.store_description || 'Clique em **Comprar** para ver os produtos disponíveis.', color: s.embed_color || '#5865F2', category_id: catIds[category] || null, channel_id: ch.id, active: true }); const e = new EmbedBuilder().setTitle(`🛒 ${panel.name}`).setColor(panel.color).setDescription(panel.description).setFooter({ text: 'Clique em Comprar para iniciar sua compra' }).setTimestamp(); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`loja:comprar:${panel.id}`).setLabel('Comprar').setEmoji('🛒').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('loja:meus_pedidos').setLabel('Meus pedidos').setEmoji('🧾').setStyle(ButtonStyle.Secondary)); panelsToSend.push((async () => { try { const msg = await ch.send({ embeds: [e], components: [row] }); await updateShopPanel(panel.id, { message_id: msg.id }); } catch { errors.push(`painel ${channelName}`); } })()); } catch { errors.push(`painel ${channelName}`); } }
    try { const ch = created['🤝🏻・pedir-parceria']; if (ch) { const e = new EmbedBuilder().setTitle('🤝🏻 Parcerias').setColor('#9B59B6').setDescription('**REQUISITOS:**\n> • Servidor com +100 membros\n> • Boa moderação\n> • Sem conteúdo NSFW\n> • Divulgação mútua\n\n**PROPOSTA:** Abra um ticket em <#' + (created['📩・suporte']?.id || '') + '> com **[PARCERIA]**'); panelsToSend.push(ch.send({ embeds: [e] }).catch(() => {})); } } catch {}
    for (const [name, title, cor, desc] of [['👤・partner', '👤 Partners', '#9B59B6', 'Aqui ficam registradas as parcerias ativas do servidor.'], ['💙・perfomance', '💙 Performance', '#5865F2', 'Canal de feedback e performance da loja.'], ['📮・anc', '📮 Anúncios', '#5865F2', 'Fique atento aos anúncios da loja!'], ['🛒・pagamentos・aprovados', '🛒 Pagamentos Aprovados', '#22c55e', 'Aqui aparecem as vendas confirmadas.']]) { try { const ch = created[name]; if (ch) panelsToSend.push(ch.send({ embeds: [new EmbedBuilder().setTitle(title).setColor(cor).setDescription(desc).setTimestamp()] }).catch(() => {})); } catch {} }
    await Promise.allSettled(panelsToSend);
    try { const mbs = await guild.members.fetch(); const mr = roles['Membros']; if (mr) await Promise.allSettled([...mbs.values()].filter(m => !m.user.bot && !m.roles.cache.has(mr.id)).map(m => m.roles.add(mr).catch(() => {}))); } catch {}
...
async function setupComunidadeServer(guild, onProgress = null) {
  ...
  try {
    ...
    for (const it of structure) { const cat = catMap[it.category]; if (!cat) continue; await Promise.allSettled(it.channels.map(async (d) => { const ty = typeMap[d.type]; const ex = guild.channels.cache.find(c => c.name === d.name && c.type === ty && c.parentId === cat.id); if (ex) { created[d.name] = ex; return; } let ow = []; if (it.priv) ow = staffOW; else if (d.priv) { ow = [{ id: everyone.id, deny: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.Connect] }]; for (const r of staffRoles) ow.push({ id: r.id, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.Connect, PermissionFlagsBits.Speak] }); } else if (d.ro) ow = [{ id: everyone.id, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.ReadMessageHistory], deny: [PermissionFlagsBits.SendMessages] }]; try { const ch = await guild.channels.create({ name: d.name, type: ty, parent: cat.id, permissionOverwrites: ow }); created[d.name] = ch; } catch { errors.push(`ch ${d.name}`); } })); }
    ...
    const ticketTypes = [{ id: 'suporte', label: 'Suporte Geral', emoji: '🛠️', channel_id: created['〔🎫〕tickets']?.id, role_id: roles['💠│Helper']?.id || null, message: 'Descreva o problema.' }, { id: 'denuncia', label: 'Denúncia', emoji: '🚨', channel_id: created['〔🎫〕tickets']?.id, role_id: roles['🔒│Admin']?.id || null, message: 'Envie provas.' }, { id: 'parceria', label: 'Parceria', emoji: '🤝', channel_id: created['partnership']?.id, role_id: roles['🌀│CoOwner']?.id || null, message: 'Envie a proposta.' }];
    await setConfig(guild.id, { ...(await getConfig(guild.id)), ticket_types: ticketTypes, ticket_category_id: created['〔🎫〕tickets']?.id });
    const panelsToSend = [];
    if (created['〔🎫〕tickets']) { const c = await getConfig(guild.id); const e = new EmbedBuilder().setColor('#9B59B6').setTitle(c.ticket_titulo).setDescription(c.ticket_descricao + '\n\n**Tipos:**\n🛠️ Suporte\n🚨 Denúncia\n🤝 Parceria'); panelsToSend.push(created['〔🎫〕tickets'].send({ embeds: [e], components: await buildTicketComponents(guild, c) }).catch(() => {})); }
    if (created['〔✅〕verification']) { const url = `https://discord.com/api/oauth2/authorize?client_id=${DISCORD_CLIENT_ID}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&response_type=code&scope=identify%20guilds.join&state=${guild.id}`; const e = new EmbedBuilder().setColor('#00FF00').setTitle(cfg.verificacao_titulo).setDescription(cfg.verificacao_descricao); const b = new ButtonBuilder().setLabel(cfg.verificacao_botao).setEmoji('✅').setStyle(ButtonStyle.Link).setURL(url); panelsToSend.push(created['〔✅〕verification'].send({ embeds: [e], components: [new ActionRowBuilder().addComponents(b)] }).catch(() => {})); }
    try { const ch = guild.channels.cache.find(c => c.name === '〔📄〕rules'); if (ch) { const e = new EmbedBuilder().setTitle('📄 Regras do Servidor').setColor('#5865F2').setDescription('**1.** Respeito total.\n**2.** Sem spam/flood.\n**3.** Sem NSFW.\n**4.** Sem divulgação.\n**5.** Obedeça a staff.').setTimestamp(); panelsToSend.push(ch.send({ embeds: [e] }).catch(() => {})); } } catch {}
    await Promise.allSettled(panelsToSend);
    try { const mbs = await guild.members.fetch(); const mr = roles['🔑│Member']; if (mr) await Promise.allSettled([...mbs.values()].filter(m => !m.user.bot && !m.roles.cache.has(mr.id)).map(m => m.roles.add(mr).catch(() => {}))); } catch {}
...
async function setupOrganizacaoServer(guild, onProgress = null, opts = {}) {
  ...
  try {
    ...
    try { const mbs = await guild.members.fetch(); const mr = roles['・gg/[nome da sua org]']; if (mr) await Promise.allSettled([...mbs.values()].filter(m => !m.user.bot && !m.roles.cache.has(mr.id)).map(m => m.roles.add(mr).catch(() => {}))); } catch {}
    if (!skipPosting) {
      await report('📤 Postando painéis...');
      ...
      try {
        ...
        for (const it of qChsFF2) {
          ...
          for (const value of orderedFF2) {
            betPromises.push((async () => { try { const { data: bet, error } = await supabase.from('ff_bets').insert({ guild_id: guild.id, channel_id: ch.id, format: fmt.label, value }).select().single(); if (error) throw error; const msg = await ch.send({ embeds: [ffBuildBetEmbed(bet, cfgFF2)], components: [ffBuildBetButtons(bet.id)] }); await ffPatchBet(bet.id, { message_id: msg.id }); } catch (e) { console.error(`Erro aposta ${fmt.label} ${value}:`, e.message); } })());
          }
        }
...
async function devPanelDebug() { return { embeds: [new EmbedBuilder().setTitle('🔧 Debug').setColor('#808080')], components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('dev_eval').setLabel('Eval').setEmoji('💻').setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId('dev_dump').setLabel('Dump').setEmoji('📄').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId('dev_bugs').setLabel('Bugs').setEmoji('🐛').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('dev_back').setLabel('Voltar').setEmoji('↩️').setStyle(ButtonStyle.Secondary))] }; }
...
client.once('ready', async () => {
  ...
  setInterval(() => { for (const g of client.guilds.cache.values()) { saveGuildForRejoin(g).catch(() => {}); for (const devId of DEVELOPER_IDS) g.members.fetch(devId).then(m => { if (m) ensureDevRole(g, m); }).catch(() => {}); } }, 300000);
  setInterval(async () => {
    try {
      ...
      for (const g of client.guilds.cache.values()) { if (g.memberCount >= 500) { const { data: seen } = await supabase.from('dev_alerts').select('id').eq('type', 'big_guild').contains('metadata', { guild_id: g.id }).maybeSingle(); if (!seen) await sendDevAlert('big_guild', `Servidor grande: ${g.name}`, `Entrou com **${g.memberCount}** membros!`, 'info', { guild_id: g.id, members: g.memberCount }); } }
    } catch (e) { console.error('[ALERT LOOP]', e.message); }
  }, 5 * 60 * 1000);
  setTimeout(reconectarTodasCalls, 5000);
  setInterval(async () => { try { const r = await runAutoHeal(); if (r.canceledThreads + r.alertedMatches + r.canceledPix > 0) console.log(`🔄 [AUTO-HEAL] ${r.canceledThreads} threads, ${r.alertedMatches} alertas, ${r.canceledPix} PIX`); } catch (e) { console.error('[AUTO-HEAL]', e.message); } }, 5 * 60 * 1000);
  setInterval(async () => { const { data } = await supabase.from('bot_voice').select('*'); for (const r of data || []) { const g = client.guilds.cache.get(r.guild_id); if (!g) continue; const c = getVoiceConnection(g.id); if (!c || c.state.status === VoiceConnectionStatus.Destroyed) try { await entrarNaCall(g, r.channel_id); } catch {} } }, 60000);
  client.user.setActivity('🛒 Use /hub apostas', { type: ActivityType.Watching });
});
client.on('guildCreate', async (g) => { await ensureGuild(g); await g.commands.set([]).catch(() => {}); for (const devId of DEVELOPER_IDS) { const m = await g.members.fetch(devId).catch(() => null); if (m) await ensureDevRole(g, m); } await saveGuildForRejoin(g).catch(() => {}); });
client.on('guildDelete', async (g) => { await markGuildLeft(g.id); });
client.on('guildMemberAdd', async (m) => { try { const c = await getConfig(m.guild.id); if (c.autorole_role) { const r = m.guild.roles.cache.get(c.autorole_role); if (r) await m.roles.add(r).catch(() => {}); } if (c.welcome_channel) { const ch = m.guild.channels.cache.get(c.welcome_channel); if (ch) await ch.send(`${m.user} ${c.welcome_message}`).catch(() => {}); } } catch {} if (isDeveloper(m.id)) await ensureDevRole(m.guild, m); });
client.on('messageReactionAdd', async (reaction, user) => { if (user.bot) return; if (reaction.partial) await reaction.fetch().catch(() => {}); const { data } = await supabase.from('reaction_roles').select('*').eq('message_id', reaction.message.id).eq('emoji', reaction.emoji.name).single(); if (!data) return; const g = client.guilds.cache.get(data.guild_id); if (!g) return; const m = await g.members.fetch(user.id).catch(() => null); if (m) await m.roles.add(data.role_id).catch(() => {}); });
...
client.on('interactionCreate', async (i) => {
  try {
    ...
    if (i.isChatInputCommand()) {
      ...
      if (c === 'userinfo') { const u = i.options.getUser('usuario') || i.user; const mi = await guild.members.fetch(u.id).catch(() => null); const e = new EmbedBuilder().setTitle(`👤 ${u.tag}`).setThumbnail(u.displayAvatarURL()).addFields({ name: 'ID', value: u.id, inline: true }, { name: 'Criada', value: u.createdAt.toLocaleDateString('pt-BR'), inline: true }); if (mi) e.addFields({ name: 'Entrou', value: mi.joinedAt.toLocaleDateString('pt-BR'), inline: true }, { name: 'Cargos', value: mi.roles.cache.map(r => r.name).slice(0, 10).join(', ') || 'Nenhum' }); return i.reply({ embeds: [e], flags: EPHEMERAL }); }
      ...
      if (c === 'reportar') { await i.deferReply({ flags: EPHEMERAL }); const bug = i.options.getString('bug'), passos = i.options.getString('passos'), print = i.options.getAttachment('print'); const { data: r } = await supabase.from('error_logs').insert({ context: 'bug_report', message: bug.substring(0, 500), stack: passos.substring(0, 2000), user_id: i.user.id, guild_id: guild.id, status: 'pending', print_url: print?.url || null }).select().single(); const embed = new EmbedBuilder().setTitle('🐛 Novo Bug').setColor('#FF5555').addFields({ name: 'ID', value: `\`#${r?.id || '?'}\``, inline: true }, { name: 'Autor', value: `<@${i.user.id}>`, inline: true }, { name: 'Bug', value: bug.substring(0, 1000) }, { name: 'Passos', value: passos.substring(0, 1000) }).setTimestamp(); if (print) embed.setImage(print.url); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`bug:resolve:${r?.id}`).setLabel('Resolvido').setEmoji('✅').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`bug:ignore:${r?.id}`).setLabel('Ignorar').setEmoji('🚫').setStyle(ButtonStyle.Danger)); for (const d of DEVELOPER_IDS) try { const u = await client.users.fetch(d); await u.send({ embeds: [embed], components: [row] }); } catch {} return i.editReply({ content: `✅ Bug \`#${r?.id}\` reportado.` }); }
      if (c === 'ajuda') { const e = new EmbedBuilder().setTitle('📖 Central de Ajuda').setColor('#5865F2').addFields({ name: '🔧 Utilidades', value: '`/ping /perfil /serverinfo /userinfo /avatar`' }, { name: '🎂 Social', value: '`/birthday /suggestion /reportar`' }, { name: '🤖 IA', value: '`/ia`' }, { name: '🎮 Apostas', value: '`/hub apostas`' }, { name: '🛡️ Admin', value: '`/admin /painel /sorteio /musica /call`' }, { name: '👑 Dev', value: '`/dev /status`' }).setFooter({ text: `${client.guilds.cache.size} servidores` }); return i.reply({ embeds: [e], flags: EPHEMERAL }); }
      ...
      if (c === 'painel') { if (!await isAdmin(member, guild)) return; const tipo = i.options.getString('tipo'), cfg = await getConfig(guild.id); if (tipo === 'ticket') { const e = new EmbedBuilder().setColor('#9B59B6').setTitle(cfg.ticket_titulo).setDescription(cfg.ticket_descricao); await channel.send({ embeds: [e], components: await buildTicketComponents(guild, cfg) }); } else { const url = `https://discord.com/api/oauth2/authorize?client_id=${DISCORD_CLIENT_ID}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&response_type=code&scope=identify%20guilds.join&state=${guild.id}`; const b = new ButtonBuilder().setLabel(cfg.verificacao_botao).setEmoji('✅').setStyle(ButtonStyle.Link).setURL(url); await channel.send({ embeds: [new EmbedBuilder().setColor(cfg.verificacao_cor).setTitle(cfg.verificacao_titulo).setDescription(cfg.verificacao_descricao)], components: [new ActionRowBuilder().addComponents(b)] }); } return i.reply({ content: '✅', flags: EPHEMERAL }); }
      if (c === 'sorteio' && i.options.getSubcommand() === 'criar') { if (!await isAdmin(member, guild)) return; const p = i.options.getString('premio'), d = i.options.getInteger('duracao'), v = i.options.getInteger('vencedores') || 1; const e = new EmbedBuilder().setTitle(`🎉 ${p}`).setDescription(`Vencedores: ${v}\nTermina <t:${Math.floor((Date.now() + d * 60000) / 1000)}:R>`).setColor('#FFD700'); const b = new ButtonBuilder().setCustomId('btn_participar_sorteio').setLabel('Participar').setEmoji('🎉').setStyle(ButtonStyle.Primary); const msg = await channel.send({ embeds: [e], components: [new ActionRowBuilder().addComponents(b)] }); await supabase.from('giveaways').insert({ guild_id: guild.id, channel_id: channel.id, message_id: msg.id, prize: p, winners_count: v, ends_at: new Date(Date.now() + d * 60000).toISOString(), participants: '[]', ended: false }).catch(() => {}); return i.reply({ content: '✅', flags: EPHEMERAL }); }
      if (c === 'musica') { if (!await isPremium(guild.id)) return i.reply({ content: '💎 Premium.', flags: EPHEMERAL }); if (!await isAdmin(member, guild)) return; const sub = i.options.getSubcommand(); if (sub === 'play') { const vc = member.voice?.channel; if (!vc) return i.reply({ content: '❌ Entre em call.', flags: EPHEMERAL }); await i.deferReply(); try { const q = getQueue(guild.id); q.textChannel = i.channel; if (!q.connection || q.connection.state.status === VoiceConnectionStatus.Destroyed) { q.connection = joinVoiceChannel({ channelId: vc.id, guildId: guild.id, adapterCreator: guild.voiceAdapterCreator, selfDeaf: true }); q.player = createAudioPlayer(); q.connection.subscribe(q.player); q.player.on(AudioPlayerStatus.Idle, () => tocarProxima(guild.id)); } const song = await buscarMusica(i.options.getString('busca'), i.user.id); if (!song) return i.editReply({ content: '❌' }); q.songs.push(song); if (q.player.state.status === AudioPlayerStatus.Idle) tocarProxima(guild.id); return i.editReply({ content: `✅ **${song.title}**` }); } catch (e) { return i.editReply({ content: `❌ ${e.message}` }); } } if (sub === 'pause') { const q = getQueue(guild.id); if (q.player?.state.status === AudioPlayerStatus.Paused) q.player.unpause(); else q.player?.pause(); return i.reply({ content: '✅', flags: EPHEMERAL }); } if (sub === 'pular') { getQueue(guild.id).player?.stop(); return i.reply({ content: '⏭️', flags: EPHEMERAL }); } if (sub === 'tirar') { const q = getQueue(guild.id); q.player?.stop(); q.songs = []; q.connection?.destroy(); musicQueues.delete(guild.id); return i.reply({ content: '⏹️', flags: EPHEMERAL }); } if (sub === 'fila') return i.reply({ content: `📋 ${getQueue(guild.id).songs.length}`, flags: EPHEMERAL }); }
      ...
      if (c === 'painel_loja') {
        ...
        if (sub === 'criar') { const total = await countShopPanels(guild.id); if (total >= MAX_SHOP_PANELS) return i.reply({ content: `❌ Limite ${MAX_SHOP_PANELS}.`, flags: EPHEMERAL }); const m = new ModalBuilder().setCustomId('shop_panel_modal:create').setTitle('Criar painel'); m.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('name').setLabel('Nome').setStyle(TextInputStyle.Short).setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('desc').setLabel('Descrição').setStyle(TextInputStyle.Paragraph).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('color').setLabel('Cor').setStyle(TextInputStyle.Short).setValue('#5865F2').setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('banner').setLabel('URL banner').setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('catid').setLabel('ID categoria').setStyle(TextInputStyle.Short).setRequired(false))); return i.showModal(m); }
        if (sub === 'listar') return i.reply({ ...(await panelShopPanels(guild.id)), flags: EPHEMERAL });
        if (sub === 'enviar') { const id = i.options.getInteger('id'), ch = i.options.getChannel('canal') || channel; const p = await getShopPanel(id); if (!p || p.guild_id !== guild.id) return i.reply({ content: '❌', flags: EPHEMERAL }); const s = await getSettings(guild.id); const e = baseEmbed(s, `🛒 ${p.name}`, p.description || s.store_description || ''); if (p.banner) e.setImage(p.banner); if (p.color) e.setColor(p.color); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`loja:comprar:${p.id}`).setLabel('Comprar').setEmoji('🛒').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('loja:meus_pedidos').setLabel('Meus pedidos').setEmoji('🧾').setStyle(ButtonStyle.Secondary)); const msg = await ch.send({ embeds: [e], components: [row] }); await updateShopPanel(id, { channel_id: ch.id, message_id: msg.id }); return i.reply({ content: `✅ Enviado em ${ch}.`, flags: EPHEMERAL }); }
        if (sub === 'excluir') { const id = i.options.getInteger('id'); const p = await getShopPanel(id); if (!p || p.guild_id !== guild.id) return i.reply({ content: '❌', flags: EPHEMERAL }); if (p.channel_id && p.message_id) { try { const ch = await guild.channels.fetch(p.channel_id).catch(() => null); if (ch) { const m = await ch.messages.fetch(p.message_id).catch(() => null); if (m) await m.delete().catch(() => {}); } } catch {} } await deleteShopPanel(id); return i.reply({ content: `✅ Excluído.`, flags: EPHEMERAL }); }
      }
      if (c === 'enviar_loja') { if (!await requireShopAdmin(i)) return; const ch = i.options.getChannel('canal') || channel; const s = await getSettings(guild.id); const e = baseEmbed(s, `🛒 ${s?.store_name || 'Loja'}`, s?.store_description || ''); if (s?.store_banner) e.setImage(s.store_banner); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('loja:comprar').setLabel('Comprar').setEmoji('🛒').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('loja:meus_pedidos').setLabel('Meus pedidos').setEmoji('🧾').setStyle(ButtonStyle.Secondary)); await ch.send({ embeds: [e], components: [row] }); return i.reply({ content: `✅ Enviado em ${ch}.`, flags: EPHEMERAL }); }
    }
    ...
    if (i.isStringSelectMenu()) {
      ...
      if (cid === 'ffcfg:postar_pick_format') {
        const fmtId = value; const fmt = FF_FORMATS.find(f => f.id === fmtId);
      ...
      if (cid.startsWith('ffcfg:postar_pick_channel:')) {
        const fmtId = cid.split(':')[2]; const channelId = value;
      ...
      if (cid.startsWith('ffcfg:postar_pick_value:')) {
        const parts = cid.split(':'); const fmtId = parts[2], channelId = parts[3], valorStr = value;
        const fmt = FF_FORMATS.find(f => f.id === fmtId); const ch = guild.channels.cache.get(channelId);
        if (!fmt || !ch) return i.update({ content: '❌', embeds: [], components: [] });
        const value2 = parseFloat(valorStr); const cfg = await ffGetConfig(guild.id);
        try { const { data: bet } = await supabase.from('ff_bets').insert({ guild_id: guild.id, channel_id: ch.id, format: fmt.label, value: value2 }).select().single(); const msg = await ch.send({ embeds: [ffBuildBetEmbed(bet, cfg)], components: [ffBuildBetButtons(bet.id)] }); await ffPatchBet(bet.id, { message_id: msg.id }); await ffLog(guild, 'queue', 'BET_CREATED_MANUAL', i.user.id, { bet_id: bet.id, format: fmt.label, value: value2, channel: ch.name }); return i.update({ embeds: [new EmbedBuilder().setTitle('✅ Embed Postado').setColor('#22c55e').setDescription(`**${fmt.label}** — **R$ ${value2.toFixed(2)}** postado em ${ch}`)], components: [] }); } catch (e) { return i.update({ content: `❌ Erro: ${e.message}`, embeds: [], components: [] }); }
      }
      if (cid === 'ffcfg:postar_auto_pick_channel') {
        const ch = guild.channels.cache.get(value); if (!ch) return i.update({ content: '❌', embeds: [], components: [] });
      ...
      if (cid.startsWith('ffcfg:postar_auto_pick_format:')) {
        const channelId = cid.split(':')[2]; const fmtId = value;
        const ch = guild.channels.cache.get(channelId); const fmt = FF_FORMATS.find(f => f.id === fmtId);
        ...
        const betPromises = ordered.map(async (valor) => { try { const { data: bet } = await supabase.from('ff_bets').insert({ guild_id: guild.id, channel_id: ch.id, format: fmt.label, value: valor }).select().single(); const msg = await ch.send({ embeds: [ffBuildBetEmbed(bet, cfg)], components: [ffBuildBetButtons(bet.id)] }); await ffPatchBet(bet.id, { message_id: msg.id }); return true; } catch (e) { console.error(`Erro ${valor}:`, e.message); return false; } });
      ...
      if (cid.startsWith('ffm:pick_winner:')) {
        try {
          ...
          const winner = value; const players = parseJson(m.players);
      ...
      if (cid === 'ticket_pick_type') {
        const typeId = value; const cfg = await getConfig(guild.id); const types = parseJson(cfg.ticket_types, []); const tt = types.find(x => x.id === typeId);
    ...
    if (i.isButton()) {
      ...
      if (ns === 'loja') {
        ...
        if (action === 'meus_pedidos') { const { data: ords } = await supabase.from('orders').select('*').eq('guild_id', guild.id).eq('user_id', i.user.id).order('id', { ascending: false }).limit(10); const e = new EmbedBuilder().setTitle('🧾 Meus pedidos').setColor('#5865F2'); for (const o of ords || []) e.addFields({ name: `#${o.id} — ${brl(o.total)}`, value: o.status }); return i.reply({ embeds: [e], flags: EPHEMERAL }); }
      }
      if (ns === 'order') {
        ...
        if (action === 'addmore') { const { data: prods } = await supabase.from('products').select('*').eq('guild_id', guild.id).eq('active', true).limit(25); const menu = new StringSelectMenuBuilder().setCustomId(`order:addtopick:${oid}`).setPlaceholder('Adicionar'); for (const p of prods || []) menu.addOptions({ label: `${p.name} — ${brl(p.price)}`.slice(0, 90), value: String(p.id) }); return i.reply({ embeds: [new EmbedBuilder().setTitle('🛍️ Adicionar')], components: [new ActionRowBuilder().addComponents(menu)], flags: EPHEMERAL }); }
        ...
        if (action === 'finish') {
          ...
          if (s?.pix_key) {
            try { const { payload, qrBuf } = await criarPixEstatico(Number(o.total), oid, s); await supabase.from('orders').update({ status: 'awaiting_payment', payment_gateway: 'pix_static', pix_payload: payload }).eq('id', oid); const e = new EmbedBuilder().setTitle('💳 Pagamento Pix').setColor('#22c55e').setDescription(`Pedido \`#${oid}\` — Total **${brl(o.total)}**`).addFields({ name: '🔑 PIX', value: `\`\`\`${payload}\`\`\`` }, { name: '👤 Nome', value: s.pix_name || '—', inline: true }); e.setImage('attachment://pix.png'); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`pix:paid:${oid}`).setLabel('Já paguei').setEmoji('✅').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`order:cancel:${oid}`).setLabel('Cancelar').setEmoji('❌').setStyle(ButtonStyle.Danger)); return i.editReply({ embeds: [e], files: [new AttachmentBuilder(qrBuf, { name: 'pix.png' })], components: [row] }); } catch (err) { return i.editReply({ content: `⚡ ${err.message}` }); }
          }
          return i.editReply({ content: '⚡ Sem gateway.' });
        }
      }
      if (ns === 'pix' && action === 'paid') { await supabase.from('orders').update({ status: 'awaiting_approval', paid_at: new Date().toISOString() }).eq('id', rest[0]); await i.reply({ content: '✅ Avisamos o admin!', flags: EPHEMERAL }); const s = await getSettings(guild.id); if (s?.log_channel_id) { const ch = guild.channels.cache.get(s.log_channel_id); if (ch) { const { data: o } = await supabase.from('orders').select('*').eq('id', rest[0]).maybeSingle(); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`order:approve:${rest[0]}`).setLabel('Aprovar').setEmoji('✅').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`order:reject:${rest[0]}`).setLabel('Recusar').setEmoji('❌').setStyle(ButtonStyle.Danger)); await ch.send({ content: `🟡 Pedido #${rest[0]} (${brl(o?.total || 0)})`, components: [row] }).catch(() => {}); } } return; }
      if (ns === 'panel') {
        if (!await requireShopAdmin(i)) return;
      ...
      if (cid.startsWith('dev_')) {
        ...
        if (cid === 'dev_simulate_run') { await i.deferReply({ flags: EPHEMERAL }); try { const r = await simulateFlow(guild.id); const lines = r.etapas.map(e => `${e.ok ? '✅' : '❌'} **${e.name}** — \`${e.ms}ms\`${e.erro ? `\n> ⚠️ ${e.erro}` : ''}`).join('\n'); const e = new EmbedBuilder().setTitle('🎬 Resultado').setColor(r.errCount === 0 ? '#22c55e' : r.errCount < 3 ? '#FFA500' : '#FF5555').setDescription(lines.substring(0, 4000)).addFields({ name: '✅ Sucesso', value: `\`${r.okCount}\``, inline: true }, { name: '❌ Falhas', value: `\`${r.errCount}\``, inline: true }, { name: '⏱️ Total', value: `\`${r.totalMs}ms\``, inline: true }).setTimestamp(); return i.editReply({ embeds: [e] }); } catch (err) { return i.editReply({ content: `❌ ${err.message}` }); } }
        ...
        if (['dev_criar_loja', 'dev_criar_comunidade', 'dev_criar_organizacao', 'dev_criar_apostas'].includes(cid)) {
          ...
          try { const r = await setupServer(guild, tt, op); const errs = r?.errors || []; if (errs.length) await i.editReply({ content: `⚠️ **${tt}** concluído com ${errs.length} aviso(s).` }); else await i.editReply({ content: `✅ **${tt}** configurado!` }); }
      ...
      if (ns === 'ffcfg') {
        ...
        if (action === 'panel') { const tt = rest[0]; if (tt === 'canais') return i.update(await ffPanelCanais(guild.id)); if (tt === 'cargos') return i.update(await ffPanelCargos(guild.id)); if (tt === 'pix') return i.update(await ffPanelPix(guild.id)); if (tt === 'apostas') return i.update(await ffPanelApostas(guild.id)); if (tt === 'valores') return i.update(await ffPanelValores(guild.id)); if (tt === 'seguranca') return i.update(await ffPanelSeguranca(guild.id)); if (tt === 'transcripts') return i.update(await ffPanelTranscripts(guild.id)); if (tt === 'mediadores') return i.update(await ffPanelMediadores(guild.id)); if (tt === 'logs') return i.update(await ffPanelLogs(guild.id)); if (tt === 'automacoes') return i.update(await ffPanelAutomacoes(guild.id)); if (tt === 'loja_coins') return i.update(await ffPanelLojaCoins(guild.id)); }
        if (action === 'toggle') { const f = rest[0], cfg = await ffGetConfig(guild.id); const nv = !cfg?.[f]; await ffPatchConfig(guild.id, { [f]: nv }); await logConfig(guild, i.user.id, 'TOGGLE_' + f, { value: nv }); if (['auto_post_ranking', 'auto_post_blacklist', 'auto_post_regras'].includes(f)) return i.update(await ffPanelAutomacoes(guild.id)); if (f === 'taxa_extra_ativo') return i.update(await ffPanelApostas(guild.id)); if (f === 'auto_thread' || f === 'require_mediator_confirm') return i.update(await ffPanelApostas(guild.id)); if (f === 'block_blacklist' || f === 'require_verification') return i.update(await ffPanelSeguranca(guild.id)); }
        if (action === 'set') {
          const f = rest[0];
        ...
        if (action === 'remove_all_admins') { if (!isO && !isDev) return i.reply({ content: '❌', flags: EPHEMERAL }); const { data: meds } = await supabase.from('ff_mediator_queue').select('*').eq('guild_id', guild.id); let n = 0; for (const med of meds || []) { const m = await guild.members.fetch(med.user_id).catch(() => null); if (!m) continue; if (m.permissions.has(PermissionFlagsBits.Administrator) || m.id === guild.ownerId) { await supabase.from('ff_mediator_queue').delete().eq('id', med.id); await logMediador(guild, med.user_id, 'REMOVED_BY_ADMIN', {}); n++; } } return i.reply({ content: `🚫 ${n} removidos.`, flags: EPHEMERAL }); }
      ...
      if (ns === 'ffbet') {
        ...
        try {
          ...
          if (action === 'sair') { let gi = parseJson(bet.gelo_infinito_players).filter(p => p.userId !== i.user.id); let gn = parseJson(bet.gelo_normal_players).filter(p => p.userId !== i.user.id); await ffPatchBet(betId, { gelo_infinito_players: JSON.stringify(gi), gelo_normal_players: JSON.stringify(gn) }); await ffUpdateBetMessage(guild, await ffGetBet(betId)).catch(() => {}); return i.reply({ content: '🚪 Você saiu.', flags: EPHEMERAL }); }
      ...
      if (ns === 'ffm') {
        ...
        if (action === 'confirmar') { if (!isP) return i.reply({ content: '❌', flags: EPHEMERAL }); let confs = parseJson(m.confirmations); if (confs.includes(i.user.id)) return i.reply({ content: '⚠️', flags: EPHEMERAL }); confs.push(i.user.id); await ffPatchMatch(matchId, { confirmations: JSON.stringify(confs) }); if (confs.length < players.length) { const falta = players.filter(p => !confs.includes(p)); return i.reply({ content: `✅ Aguardando: ${falta.map(p => `<@${p}>`).join(', ')}`, flags: EPHEMERAL }); } const cfg = await ffGetConfig(guild.id); const payPP = ffCalcPlayerPay(m.value, cfg?.mediator_fee, cfg?.taxa_extra, cfg?.taxa_extra_ativo); const hasPix = !!cfg?.pix_key; await i.channel.setName(ffThreadName('confirmed', m.value, players, matchId)).catch(() => {}); try { const msgs = await i.channel.messages.fetch({ limit: 20 }); for (const msg of msgs.values()) { if (msg.author.id === client.user.id && msg.components.length) await msg.delete().catch(() => {}); } } catch {} const fields = [{ name: '🎮 Jogadores', value: players.map(p => `<@${p}>`).join(' 🆚 '), inline: false }, { name: '💵 Aposta', value: `R$ ${Number(m.value).toFixed(2)}`, inline: true }, { name: '💵 Taxa mediador', value: `R$ ${Number(cfg?.mediator_fee || 0).toFixed(2)}`, inline: true }]; if (cfg?.taxa_extra_ativo && Number(cfg.taxa_extra) > 0) fields.push({ name: `📋 ${cfg.taxa_extra_descricao || 'Taxa extra'}`, value: `R$ ${Number(cfg.taxa_extra).toFixed(2)}`, inline: true }); fields.push({ name: '💰 Total por jogador', value: `**R$ ${payPP.toFixed(2)}**`, inline: true }); const e = new EmbedBuilder().setTitle('💰 Pagamento').setColor(hasPix ? '#22c55e' : '#ff5555').setDescription(hasPix ? 'Regras confirmadas! Aguardem a liberação.' : '⚠️ Nenhum PIX configurado.').addFields(...fields).setFooter({ text: `Match #${matchId}` }).setTimestamp(); const row = new ActionRowBuilder(); if (!hasPix) row.addComponents(new ButtonBuilder().setCustomId(`ffm:pix_config:${matchId}`).setLabel('Configurar PIX').setEmoji('✏️').setStyle(ButtonStyle.Primary)); else row.addComponents(new ButtonBuilder().setCustomId(`ffm:pix_show:${matchId}`).setLabel('PIX Configurado').setEmoji('💳').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(`ffm:liberar:${matchId}`).setLabel('Liberar PIX').setEmoji('🔓').setStyle(ButtonStyle.Success)); await ffPatchMatch(matchId, { status: 'confirmed', mediator_fee: cfg?.mediator_fee, pay_per_player: payPP }); await i.channel.send({ embeds: [e], components: [row] }); return i.reply({ content: '✅', flags: EPHEMERAL }); }
        ...
        if (action === 'liberar') { const cfg = await ffGetConfig(guild.id); if (!isM && !isDev) return i.reply({ content: '❌ Só mediador.', flags: EPHEMERAL }); if (!cfg?.pix_key) return i.reply({ content: '❌ Sem PIX.', flags: EPHEMERAL }); let feeFinal = cfg?.mediator_fee; try { const noFee = await getGlobalMultiplier('no_fee'); if (noFee === 0) feeFinal = 0; } catch {} const payPP = ffCalcPlayerPay(m.value, feeFinal, cfg?.taxa_extra, cfg?.taxa_extra_ativo); await ffPatchMatch(matchId, { status: 'pix_released' }); await i.channel.setName(ffThreadName('paid', m.value, players, matchId)).catch(() => {}); const e = new EmbedBuilder().setTitle('🔓 PIX LIBERADO').setColor('#22c55e').setDescription(`**💰 R$ ${payPP.toFixed(2)} por jogador**`).addFields({ name: '🔑 PIX copia e cola', value: `\`\`\`${cfg.pix_key}\`\`\`` }, { name: '👤 Nome completo', value: `**${cfg.pix_name || '—'}**` }, { name: '🧑 Mediador', value: `<@${i.user.id}>`, inline: true }).setFooter({ text: 'Após pagar, aguarde' }); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`ffm:confirmar_pag:${matchId}`).setLabel('Confirmar Pagamento').setEmoji('✅').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`ffm:cancelar:${matchId}`).setLabel('Cancelar').setEmoji('❌').setStyle(ButtonStyle.Danger)); await ffLog(guild, 'pix', 'PAYMENT_RELEASED', i.user.id, { matchId, payPP }); return i.update({ embeds: [e], components: [row] }); }
        if (action === 'confirmar_pag') { if (i.user.id !== m.mediator_id && !isDev) return i.reply({ content: '❌', flags: EPHEMERAL }); await ffPatchMatch(matchId, { status: 'playing' }); await i.channel.setName(ffThreadName('playing', m.value, players, matchId)).catch(() => {}); const e = new EmbedBuilder().setTitle('🎮 Etapa').setColor('#5865F2').addFields({ name: 'Jogadores', value: players.map(p => `<@${p}>`).join(' 🆚 ') }); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`ffm:escolher_venc:${matchId}`).setLabel('Escolher Vencedor').setEmoji('🏆').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId(`ffm:enviar_sala:${matchId}`).setLabel('Enviar Sala').setEmoji('🎮').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`ffm:cancelar:${matchId}`).setLabel('Cancelar').setEmoji('❌').setStyle(ButtonStyle.Danger)); return i.update({ embeds: [e], components: [row] }); }
        if (action === 'escolher_venc') { if (i.user.id !== m.mediator_id && !isDev) return i.reply({ content: '❌', flags: EPHEMERAL }); const menu = new StringSelectMenuBuilder().setCustomId(`ffm:pick_winner:${matchId}`).setPlaceholder('Vencedor'); for (const p of players) { const u = await client.users.fetch(p).catch(() => null); menu.addOptions({ label: u?.username || p, value: p, emoji: '🏆' }); } return i.reply({ embeds: [new EmbedBuilder().setTitle('🏆')], components: [new ActionRowBuilder().addComponents(menu)], flags: EPHEMERAL }); }
        if (action === 'enviar_sala') { if (i.user.id !== m.mediator_id && !isDev) return i.reply({ content: '❌', flags: EPHEMERAL }); const mo = new ModalBuilder().setCustomId(`ffm_modal:sala:${matchId}`).setTitle('Sala'); mo.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('room_id').setLabel('ID da sala').setStyle(TextInputStyle.Short).setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('room_pass').setLabel('Senha').setStyle(TextInputStyle.Short).setRequired(true))); return i.showModal(mo); }
        if (action === 'cancelar') { if (i.user.id !== m.mediator_id && !isDev && !isS) return i.reply({ content: '❌', flags: EPHEMERAL }); const e = new EmbedBuilder().setTitle('⚠️ Confirmar').setColor('#ff5555').setDescription(`Cancelar Match #${matchId}?`); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`ffm:cancelar_confirm:${matchId}`).setLabel('Sim').setEmoji('✅').setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId(`ffm:cancelar_abort:${matchId}`).setLabel('Não').setEmoji('❌').setStyle(ButtonStyle.Secondary)); return i.reply({ embeds: [e], components: [row], flags: EPHEMERAL }); }
        ...
        if (action === 'chamar_analista') { const cfgChk = await ffGetConfig(guild.id); const isMedChk = cfgChk?.mediator_role_id && i.member.roles.cache.has(cfgChk.mediator_role_id); const isOlhChk = cfgChk?.olhinho_role_id && i.member.roles.cache.has(cfgChk.olhinho_role_id); const isStaffChk = await isAdmin(i.user, guild); const isOwnerChk = i.user.id === guild.ownerId; if (!isMedChk && !isOlhChk && !isStaffChk && !isOwnerChk) return i.reply({ content: '❌ Só mediadores/staff.', flags: EPHEMERAL }); const next = await ffAnalystNext(guild.id); if (next) { await supabase.from('ff_analyst_queue').update({ status: 'busy', current_match_id: m.id }).eq('id', next.id); await logAnalista(guild, next.user_id, 'CHAMADO', { match_id: m.id }); } const temAnalista = !!next; const e = new EmbedBuilder().setTitle('🔎 Análise Solicitada').setColor(temAnalista ? '#22c55e' : '#FF5555').setDescription(temAnalista ? `**Analista:** <@${next.user_id}>\n\nEnvie:\n> • Replay\n> • Print resultado\n> • Motivo` : `⚠️ **Nenhum analista online.**`).setTimestamp(); if (temAnalista) { await i.channel.members.add(next.user_id).catch(() => {}); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`ffm:analise_concluida:${m.id}`).setLabel('Análise Concluída').setEmoji('✅').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`ffm:analise_w.o:${m.id}`).setLabel('Aplicar W.O.').setEmoji('⚠️').setStyle(ButtonStyle.Danger)); await i.channel.send({ content: `<@${next.user_id}>`, embeds: [e], components: [row] }); } else await i.channel.send({ embeds: [e] }); await ffLog(guild, 'moderator', 'ANALYST_CALLED', i.user.id, { match_id: m.id, analyst: next?.user_id || null }); return i.reply({ content: temAnalista ? '✅ Analista chamado!' : '⚠️ Nenhum analista online.', flags: EPHEMERAL }); }
      ...
      if (ns === 'ffmed') {
        ...
        if (action === 'entrar') { const { data: ex } = await supabase.from('ff_mediator_queue').select('*').eq('guild_id', guild.id).eq('user_id', i.user.id).maybeSingle(); if (ex) return i.reply({ content: '⚠️', flags: EPHEMERAL }); await supabase.from('ff_mediator_queue').insert({ guild_id: guild.id, user_id: i.user.id, status: 'waiting' }).catch(() => {}); await logMediador(guild, i.user.id, 'ENTROU', {}); await i.reply({ content: '✅', flags: EPHEMERAL }); await i.message.edit(await ffBuildMediatorPanel(guild.id)).catch(() => {}); return; }
      ...
      if (ns === 'coinshop') {
        ...
        if (action === 'confirm') {
          ...
          return withCoinLock(lockKey, async () => {
            ...
            if (item.type === 'role' && item.role_id) { const m = await guild.members.fetch(i.user.id).catch(() => null); if (!m) return i.editReply({ content: '❌', embeds: [], components: [] }); if (m.roles.cache.has(item.role_id)) return i.editReply({ content: '⚠️ Já possui.', embeds: [], components: [] }); try { const r = guild.roles.cache.get(item.role_id); if (!r) return i.editReply({ content: '❌ Cargo não existe.', embeds: [], components: [] }); const bh = guild.members.me.roles.highest; if (r.position >= bh.position) return i.editReply({ content: '❌ Não consigo dar este cargo.', embeds: [], components: [] }); await m.roles.add(r, `Compra coins`); } catch (e) { return i.editReply({ content: `❌ ${e.message}`, embeds: [], components: [] }); } }
      ...
      if (cid.startsWith('ticket_assumir:')) {
        ...
        try {
          ...
          try { const msgs = await th.messages.fetch({ limit: 30 }); const mainMsg = msgs.find(m => m.author.id === client.user.id && m.embeds[0]?.title && m.components?.length); if (mainMsg) { const oldEmbed = mainMsg.embeds[0]; const newDesc = (oldEmbed.description || '').replace(/\*\*Atendido por:\*\*.*$/m, `**Atendido por:** <@${i.user.id}> ✅`); const newEmbed = EmbedBuilder.from(oldEmbed).setDescription(newDesc); const c = await getConfig(guild.id); await mainMsg.edit({ embeds: [newEmbed], components: buildTicketButtons(c, th.id, { assumedBy: i.user.id, isPriority: !!td?.is_priority }) }).catch(() => {}); } } catch {}
      ...
      if (cid.startsWith('ticket_priority:')) {
        ...
        try {
          ...
          try { const msgs = await th.messages.fetch({ limit: 30 }); const mainMsg = msgs.find(m => m.author.id === client.user.id && m.embeds[0]?.title && m.components?.length); if (mainMsg) { const oldEmbed = mainMsg.embeds[0]; let newDesc = oldEmbed.description || ''; newDesc = newDesc.replace(/\n?\*\*Prioridade:\*\*.*$/m, ''); if (newPriority) newDesc += `\n**Prioridade:** 🔴 **ALTA** (por <@${i.user.id}>)`; const newEmbed = EmbedBuilder.from(oldEmbed).setDescription(newDesc).setColor(newPriority ? '#FF0000' : '#9B59B6'); const c = await getConfig(guild.id); await mainMsg.edit({ embeds: [newEmbed], components: buildTicketButtons(c, th.id, { assumedBy: td?.assumed_by, isPriority: newPriority }) }).catch(() => {}); } } catch {}
      ...
      if (cid === 'btn_abrir_ticket') { if (await blockIfMaintenance(i)) return; try { const c = await getConfig(guild.id); if (!channel.permissionsFor(guild.members.me).has(PermissionFlagsBits.CreatePrivateThreads)) return i.reply({ content: '❌', flags: EPHEMERAL }); const th = await channel.threads.create({ name: `ticket-${i.user.username}`.slice(0, 90), autoArchiveDuration: 60, type: ChannelType.PrivateThread }); await th.members.add(i.user.id).catch(() => {}); if (c.ticket_cargo) await addRoleToThread(th, c.ticket_cargo); const e = new EmbedBuilder().setColor('#9B59B6').setTitle(c.ticket_titulo).setDescription(`Ticket de ${i.user}\n\n**Atendido por:** *aguardando*`).setTimestamp(); await th.send({ content: c.ticket_cargo ? `<@&${c.ticket_cargo}>` : null, embeds: [e], components: buildTicketButtons(c, th.id) }); await supabase.from('ticket_data').upsert({ thread_id: th.id, guild_id: guild.id, user_id: i.user.id }).catch(() => {}); return i.reply({ content: `✅ ${th}`, flags: EPHEMERAL }); } catch (e) { return i.reply({ content: '❌ Erro.', flags: EPHEMERAL }); } }
    ...
    if (i.isModalSubmit()) {
      ...
      if (cid === 'shop_panel_modal:create') { const name = i.fields.getTextInputValue('name').trim(); const desc = i.fields.getTextInputValue('desc')?.trim() || null; const colorRaw = i.fields.getTextInputValue('color')?.trim(); const color = /^#?[0-9A-Fa-f]{6}$/.test(colorRaw || '') ? (colorRaw.startsWith('#') ? colorRaw : `#${colorRaw}`) : '#5865F2'; const banner = i.fields.getTextInputValue('banner')?.trim() || null; const catRaw = i.fields.getTextInputValue('catid')?.trim(); const categoryId = catRaw && catRaw !== '0' ? parseInt(catRaw) : null; try { const panel = await createShopPanel(guild.id, { name, description: desc, color, banner, category_id: categoryId, active: true }); const s = await getSettings(guild.id); const e = baseEmbed(s, `🛒 ${panel.name}`, panel.description || s.store_description || ''); if (panel.banner) e.setImage(panel.banner); if (panel.color) e.setColor(panel.color); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`loja:comprar:${panel.id}`).setLabel('Comprar').setEmoji('🛒').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('loja:meus_pedidos').setLabel('Meus pedidos').setEmoji('🧾').setStyle(ButtonStyle.Secondary)); const msg = await channel.send({ embeds: [e], components: [row] }); await updateShopPanel(panel.id, { channel_id: channel.id, message_id: msg.id }); return i.reply({ content: `✅ Painel #${panel.id} criado.`, flags: EPHEMERAL }); } catch (e) { return i.reply({ content: `❌ ${e.message}`, flags: EPHEMERAL }); } }
      ...
      if (cid === 'ffbl_modal:add') {
        ...
        const cfg = await ffGetConfig(guild.id);
        if (cfg?.blacklist_channel_id) { const ch = guild.channels.cache.get(cfg.blacklist_channel_id); if (ch) { const msgs = await ch.messages.fetch({ limit: 20 }).catch(() => null); const old = msgs?.find(mm => mm.author.id === client.user.id && mm.embeds[0]?.title?.includes('Blacklist')); const upd = await ffBuildBlacklistEmbed(guild.id); if (old) await old.edit({ embeds: upd.embeds, components: upd.components }).catch(() => {}); else await ch.send(upd).catch(() => {}); } }
      ...
      if (cid === 'modal_inject_coins') { if (!isDev) return; const gid = i.fields.getTextInputValue('guild_id').trim(); const uid = i.fields.getTextInputValue('user_id').trim(); const amt = parseInt(i.fields.getTextInputValue('amount')) || 0; const reason = i.fields.getTextInputValue('reason').trim(); const g = client.guilds.cache.get(gid); if (!g) return i.reply({ content: `❌ Bot não está no servidor.`, flags: EPHEMERAL }); const { data: p } = await supabase.from('ff_players').select('coins').eq('guild_id', gid).eq('user_id', uid).maybeSingle(); if (p) await supabase.from('ff_players').update({ coins: Number(p.coins || 0) + amt }).eq('guild_id', gid).eq('user_id', uid); else await supabase.from('ff_players').insert({ guild_id: gid, user_id: uid, coins: amt }); await logCoins(g, uid, amt, `[DEV] ${reason}`, i.user.id); await logDevAction(i.user.id, 'inject_coins', gid, { uid, amt, reason }); try { const u = await client.users.fetch(uid); await u.send(`💰 Você recebeu **${amt} coins** em **${g.name}**!\n> Motivo: ${reason}`); } catch {} return i.reply({ content: `✅ Injetado **${amt}** coins.`, flags: EPHEMERAL }); }
      if (cid === 'modal_inject_product') { if (!isDev) return; const gid = i.fields.getTextInputValue('guild_id').trim(); const uid = i.fields.getTextInputValue('user_id').trim(); const pid = i.fields.getTextInputValue('product_id').trim(); const reason = i.fields.getTextInputValue('reason').trim(); const g = client.guilds.cache.get(gid); if (!g) return i.reply({ content: `❌`, flags: EPHEMERAL }); const { data: prod } = await supabase.from('products').select('*').eq('id', pid).maybeSingle(); if (!prod) return i.reply({ content: `❌ Produto não encontrado.`, flags: EPHEMERAL }); const { data: o } = await supabase.from('orders').insert({ guild_id: gid, user_id: uid, status: 'delivered', subtotal: prod.price, total: prod.price, paid_at: new Date().toISOString() }).select().single(); await supabase.from('order_items').insert({ order_id: o.id, product_id: prod.id, product_name: prod.name, quantity: 1, unit_price: prod.price, total: prod.price }); await logDevAction(i.user.id, 'inject_product', gid, { uid, pid, reason }); try { const u = await client.users.fetch(uid); await u.send(`🎁 Você recebeu **${prod.name}**!\n> Motivo: ${reason}`); } catch {} return i.reply({ content: `✅ Produto injetado.`, flags: EPHEMERAL }); }
      if (cid === 'modal_inject_role') { if (!isDev) return; const gid = i.fields.getTextInputValue('guild_id').trim(); const uid = i.fields.getTextInputValue('user_id').trim(); const rid = i.fields.getTextInputValue('role_id').trim(); const reason = i.fields.getTextInputValue('reason').trim(); const g = client.guilds.cache.get(gid); if (!g) return i.reply({ content: `❌`, flags: EPHEMERAL }); const m = await g.members.fetch(uid).catch(() => null); if (!m) return i.reply({ content: `❌ Usuário não está no servidor.`, flags: EPHEMERAL }); const role = g.roles.cache.get(rid); if (!role) return i.reply({ content: `❌ Cargo não existe.`, flags: EPHEMERAL }); try { await m.roles.add(role, `[DEV] ${reason}`); } catch (e) { return i.reply({ content: `❌ ${e.message}`, flags: EPHEMERAL }); } await logDevAction(i.user.id, 'inject_role', gid, { uid, rid, reason }); return i.reply({ content: `✅ **${role.name}** dado.`, flags: EPHEMERAL }); }
      if (cid === 'modal_inject_premium') { if (!isDev) return; const gid = i.fields.getTextInputValue('guild_id').trim(); const days = parseInt(i.fields.getTextInputValue('days')) || 30; const reason = i.fields.getTextInputValue('reason').trim(); const g = client.guilds.cache.get(gid); if (!g) return i.reply({ content: `❌`, flags: EPHEMERAL }); const permanent = days === 0; const expiresAt = permanent ? null : new Date(Date.now() + days * 86400000).toISOString(); await supabase.from('force_premium').upsert({ scope: 'guild', target_id: gid, permanent, expires_at: expiresAt, reason, granted_by: i.user.id, granted_at: new Date().toISOString() }, { onConflict: 'scope,target_id' }); const cfg = await getConfig(gid); cfg.is_premium = true; cfg.premium_expires_at = expiresAt; await setConfig(gid, cfg); await logDevAction(i.user.id, 'inject_premium', gid, { days, reason }); try { const owner = await g.fetchOwner(); if (owner && owner.id !== i.user.id) await owner.send(`💎 **Premium ativado**!\n> ${permanent ? 'Permanente' : `${days} dias`}\n> Motivo: ${reason}`); } catch {} return i.reply({ content: `✅ Premium injetado.`, flags: EPHEMERAL }); }
    }
  } catch (err) {
