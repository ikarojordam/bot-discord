...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isButton()) {
    if (interaction.customId === 'btn_abrir_ticket') {
      ...
      // Não adicionar cargo, apenas o staff pode ser adicionado manualmente se necessário
      const ticketEmbed = new EmbedBuilder()
        .setColor('#9B59B6')
  ...
  if (interaction.isModalSubmit()) {
    if (interaction.customId === 'modal_vender') {
      ...
      try {
        ...
        // NÃO adicionamos cargo à thread, apenas o vendedor é removido e admins com Manage Threads veem
