...
async function getConfig(guildId) {
  ...
  if (error || !data) {
    console.log(`Nenhuma config encontrada para ${guildId}, usando default.`);
    return { guild_id: guildId, ...defaultConfig };
  }
  console.log(`Config carregada para ${guildId}:`, data);
...
async function setConfig(guildId, newConfig) {
  ...
  else console.log('Config salva com sucesso.');
}
...
// Função para obter membro completo da guilda
...
// Verificação de admin (dono sempre admin)
async function isAdmin(memberOrUser, guild) {
  // Se for um membro completo (tem roles)
...
async function isAllowed(memberOrUser, guild) {
  const userId = memberOrUser?.user?.id || memberOrUser?.id;
  if (!userId) return false;
  const member = memberOrUser?.roles ? memberOrUser : await fetchMember(guild, userId);
  if (!member) return false;
  const config = await getConfig(guild.id);
  return (config.membro_role && member.roles.cache.has(config.membro_role)) || await isAdmin(member, guild);
}

// Verificação de staff de ticket (dono ou cargo configurado)
async function isTicketStaff(memberOrUser, guild) {
  ...
  if (member.id === guild.ownerId) {
    console.log(`${member.user.tag} é dono do servidor.`);
    return true;
  }
  const config = await getConfig(guild.id);
  ...
  const hasRole = config.ticket_cargo && member.roles.cache.has(config.ticket_cargo);
  const isAdminResult = await isAdmin(member, guild);
  console.log(`hasRole=${hasRole}, isAdmin=${isAdminResult}`);
  return hasRole || isAdminResult;
}
...
async function addRoleToThread(thread, roleId) {
  ...
  if (!role) {
    console.error(`Cargo ${roleId} não encontrado no cache.`);
    return;
  }
  console.log(`Adicionando membros do cargo ${role.name} à thread ${thread.name}`);
  const members = role.members.map(m => m.id);
  console.log(`Membros do cargo: ${members.length}`);
  const results = await Promise.allSettled(
    members.map(memberId => thread.members.add(memberId).catch(e => {
      console.error(`Erro ao adicionar ${memberId}:`, e.message);
      return { error: e };
    }))
  );
  results.forEach((result, index) => {
    if (result.status === 'fulfilled') console.log(`Membro ${members[index]} adicionado com sucesso.`);
    else console.error(`Falha ao adicionar membro ${members[index]}:`, result.reason?.message || result.reason);
  });
}
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (commandName === 'vendas') {
      if (!await isAllowed(member, guild)) return interaction.reply({ content: '❌ Sem permissão.', ephemeral: true });
  ...
  if (interaction.isStringSelectMenu()) {
    ...
    if (interaction.customId.startsWith('select_role_')) {
      ...
      console.log(`Configurado ${configKey} = ${roleId}`);
  ...
  if (interaction.isButton()) {
    // Botões que abrem modal PRIMEIRO (sem deferUpdate)
    ...
    // Demais botões: deferUpdate para evitar timeout
    ...
    if (interaction.customId === 'btn_abrir_ticket') {
      try {
        ...
        console.log(`Config ticket_cargo: ${config.ticket_cargo}`);
        ...
        if (config.ticket_cargo) {
          ...
          console.log(`Menção do cargo: ${content}`);
        }
    ...
    if (interaction.customId === 'btn_fechar_ticket' || interaction.customId === 'btn_avisar_adm') {
      console.log('Botão de fechar/avisar clicado por:', interaction.user.tag);
      if (!await isTicketStaff(interaction.user, guild)) {
  ...
  if (interaction.isModalSubmit()) {
    ...
    if (interaction.customId === 'modal_vender') {
      ...
      try {
        ...
        await interaction.editReply({ content: `✅ Venda enviada! Os administradores revisarão.` });
      } catch (e) {
