...
async function isAllowed(memberOrUser, guild) {
  ...
  if (!member) return false;
  const config = await getConfig(guild.id);
  // Se nenhum cargo de membro foi configurado, permite todos
  if (!config.membro_role) return true;
  return member.roles.cache.has(config.membro_role) || await isAdmin(member, guild);
}
