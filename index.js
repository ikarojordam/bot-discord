...
async function setupServer(guild, type) {
  ...
  for (const channel of channels) {
    if (channel.deletable) {
      ...
      await new Promise(resolve => setTimeout(resolve, 500)); // Evita rate limit
    }
  }
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (commandName === 'configurar') {
      ...
      // Criar menu de seleção para canais
      ...
      const channelSelect = new StringSelectMenuBuilder()
        ...
        .setPlaceholder('Selecione o canal para painel de vendas')
        .addOptions(channelOptions);

      const roleSelect = new StringSelectMenuBuilder()
        .setCustomId('select_config_role')
      ...
      await interaction.reply({ content: '**Configuração rápida:**\n1️⃣ Escolha o canal do painel de vendas.\n2️⃣ Escolha o cargo admin.', components: [row1, row2], ephemeral: true });
      return;
    }
  ...
  if (interaction.isStringSelectMenu()) {
    if (interaction.customId === 'select_config_channel') {
    ...
    if (interaction.customId === 'select_config_role') {
  ...
  if (interaction.isButton()) {
    ...
    if (interaction.customId === 'btn_abrir_ticket') {
      try {
        ...
        // Verificar se o bot tem permissão para criar threads privadas
