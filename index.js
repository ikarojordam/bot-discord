...
client.on('interactionCreate', async (i) => {
  try {
    ...
    // ⚡ ATALHOS ULTRA-RÁPIDOS: /dev e /ping respondem IMEDIATAMENTE
    if (i.isChatInputCommand() && i.commandName === 'ping') {
      return i.reply({ content: `🏓 ${client.ws.ping}ms`, flags: EPHEMERAL }).catch(() => {});
    }
    if (i.isChatInputCommand() && i.commandName === 'dev' && isDev) {
      return i.reply({ ...devHub(), flags: EPHEMERAL }).catch(() => {});
    }

    // 🚨 KILL SWITCH (com cache de 60s → rápido)
    ...
    // 🕵️ Abuse tracker (síncrono — 0 queries)
    if (i.user?.id && i.guild) {
      if (trackAbuse(i.user.id, i.type || 'interaction', i.guild.id, 50, 10000)) {
        return i.reply({ content: '⚠️ Você está indo muito rápido. Aguarde um pouco.', flags: EPHEMERAL }).catch(() => {});
      }
    }

    // ⚡ Dev bypassa manutenção
    if (!isDev) {
    ...
    if (i.isStringSelectMenu()) {
      ...
      if (cid === 'ffcfg:postar_pick_format') {
        ...
        const chMap = {
          ...
          '1x1_emu': '💻・1x1-emu', '2x2_emu': '💻・2x2-emu', '3x3_emu': '💻・3x3-emo', '4x4_emu': '💻・4x4-emu',
      ...
      if (cid.startsWith('ffm:pick_winner:')) {
        try {
          ...
          const e = new EmbedBuilder().setTitle('🏆 FINALIZADO').setColor('#f1c40f')
            ...
            .addFields({ name: '💰 Prêmio', value: `R$ ${prize.toFixed(2)}`, inline: true }, { name: '💎 Coins', value: `${coins}`, inline: true }, { name: '💵 Taxa mediador', value: `R$ ${fee.toFixed(2)}`, inline: true }).setTimestamp();
      ...
      if (cid === 'ticket_pick_type') {
        ...
        try {
          ...
          const e = new EmbedBuilder().setColor('#9B59B6').setTitle(`${t.emoji || '🎫'} ${t.label}`)
            .setDescription(`${t.message || 'Aguarde atendimento.'}\n\n**Aberto por:** <@${i.user.id}>\n**Categoria:** \`${t.label}\`\n**Atendido por:** *aguardando*`)
    ...
    // ─── SELECT MENUS (Channels/Roles/Users) ──────────────
    ...
    if (i.isButton()) {
      ...
      if (cid.startsWith('adm_') || ns === 'adm') {
        ...
        if (cid === 'adm_maint_toggle') {
          ...
          try { const ch = guild.channels.cache.find(c => c.name.includes('anuncio')); if (ch) await ch.send({ embeds: [new EmbedBuilder().setTitle(nv ? '🔧 MANUTENÇÃO ADMIN ATIVA' : '🟢 ADMIN LIBERADO').setColor(nv ? '#ff5555' : '#22c55e').setDescription(nv ? 'Comandos administrativos desabilitados.' : 'Comandos administrativos liberados.').addFields({ name: 'Por', value: `<@${i.user.id}>` }).setTimestamp()] }); } catch {}
        ...
        if (cid === 'adm_maint_reason') { const m = new ModalBuilder().setCustomId('adm_maint_reason_modal').setTitle('Motivo da Manutenção'); m.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('r').setLabel('Motivo').setStyle(TextInputStyle.Paragraph).setRequired(true).setMaxLength(300))); return i.showModal(m); }
      ...
      if (cid.startsWith('dev_')) {
        ...
        if (cid === 'dev_alerts_config') {
          ...
          for (let idx = 0; idx < tipos.length; idx++) {
            const t = tipos[idx]; const enabled = cfgMap[t] !== false;
        ...
        if (cid.startsWith('dev_alert_toggle:')) {
          ...
          return i.reply({ content: `✅ ${t}: ${nv ? '🟢' : '🔴'}`, flags: EPHEMERAL });
        }
        if (cid === 'dev_alerts_test') { await sendDevAlert('test', 'Alerta de teste', 'Alerta disparado manualmente.', 'info', { test: true, by: i.user.id }); return i.reply({ content: '✅ Alerta enviado para todos os devs.', flags: EPHEMERAL }); }
        ...
        if (cid.startsWith('dev_staff_bl_add:')) {
          ...
          if (ex) { await supabase.from('staff_blacklist').delete().eq('user_id', uid); return i.reply({ content: `✅ Removido da blacklist.`, flags: EPHEMERAL }); }
        ...
        if (cid === 'dev_forcepremium_user') { const m = new ModalBuilder().setCustomId('modal_forcepremium_user').setTitle('ForcePremium Usuário'); m.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('user_id').setLabel('ID user').setStyle(TextInputStyle.Short).setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('guild_id').setLabel('ID guild (opcional)').setStyle(TextInputStyle.Short).setRequired(false)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('days').setLabel('Dias (0=perm)').setStyle(TextInputStyle.Short).setValue('0').setRequired(true)), new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('reason').setLabel('Motivo').setStyle(TextInputStyle.Paragraph).setRequired(false).setMaxLength(200))); return i.showModal(m); }
        if (cid === 'dev_forcepremium_list') { const { data } = await supabase.from('force_premium').select('*').order('granted_at', { ascending: false }).limit(30); if (!data?.length) return i.reply({ content: '📋 Nenhum ativo.', flags: EPHEMERAL }); const lines = data.map(f => { const exp = f.permanent ? '♾️ perm' : (f.expires_at ? `<t:${Math.floor(new Date(f.expires_at).getTime() / 1000)}:R>` : '?'); return `**${f.scope === 'guild' ? '🌐' : '👤'}** \`${f.target_id}\`\n> ${exp} • <@${f.granted_by}>${f.reason ? ` • *${f.reason}*` : ''}`; }); return i.reply({ embeds: [new EmbedBuilder().setTitle('🎯 ForcePremium — Ativos').setColor('#FFD700').setDescription(lines.join('\n\n').substring(0, 4000))], flags: EPHEMERAL }); }
        if (cid === 'dev_forcepremium_clear') { const { count } = await supabase.from('force_premium').select('*', { count: 'exact', head: true }); if (!count) return i.reply({ content: '📋 Nada.', flags: EPHEMERAL }); return i.reply({ embeds: [new EmbedBuilder().setTitle('⚠️ Confirmar').setColor('#ff5555').setDescription(`Apagar **${count}** registros?`)], components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('dev_forcepremium_clear_confirm').setLabel('Sim').setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId('dev_back').setLabel('Cancelar').setStyle(ButtonStyle.Secondary))], flags: EPHEMERAL }); }
      ...
      if (ns === 'ffm') {
        ...
        if (action === 'cancelar') { if (i.user.id !== m.mediator_id && !isDeveloper(i.user.id) && !isS) return i.reply({ content: '❌', flags: EPHEMERAL }); const e = new EmbedBuilder().setTitle('⚠️ Confirmar').setColor('#ff5555').setDescription(`Cancelar Match #${matchId}?`); const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`ffm:cancelar_confirm:${matchId}`).setLabel('Sim').setEmoji('✅').setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId(`ffm:cancelar_abort:${matchId}`).setLabel('Não').setEmoji('❌').setStyle(ButtonStyle.Secondary)); return i.reply({ embeds: [e], components: [row], flags: EPHEMERAL }); }
        ...
        if (action === 'chamar_analista') {
          ...
          const e = new EmbedBuilder().setTitle('🔎 Análise Solicitada').setColor(temAnalista ? '#22c55e' : '#FF5555')
            .setDescription(temAnalista ? `**Analista marcado:** <@${next.user_id}>\n\nEnvie:\n> • Replay da partida\n> • Print do resultado\n> • Motivo da análise` : `⚠️ **Não tem nenhum analista online.**`).setTimestamp();
          if (temAnalista) {
      ...
      // ─── TICKET — ASSUMIR / PRIORIDADE / ABRIR / FECHAR ─────
    ...
    if (i.isModalSubmit()) {
      ...
      if (cid === 'modal_inject_product') { if (!isDev) return; const gid = i.fields.getTextInputValue('guild_id').trim(); const uid = i.fields.getTextInputValue('user_id').trim(); const pid = i.fields.getTextInputValue('product_id').trim(); const reason = i.fields.getTextInputValue('reason').trim(); const g = client.guilds.cache.get(gid); if (!g) return i.reply({ content: `❌`, flags: EPHEMERAL }); const { data: prod } = await supabase.from('products').select('*').eq('id', pid).maybeSingle(); if (!prod) return i.reply({ content: `❌ Produto não encontrado.`, flags: EPHEMERAL }); const { data: o } = await supabase.from('orders').insert({ guild_id: gid, user_id: uid, status: 'delivered', subtotal: prod.price, total: prod.price, paid_at: new Date().toISOString() }).select().single(); await supabase.from('order_items').insert({ order_id: o.id, product_id: prod.id, product_name: prod.name, quantity: 1, unit_price: prod.price, total: prod.price }); await logDevAction(i.user.id, 'inject_product', gid, { uid, pid, reason }); try { const u = await client.users.fetch(uid); await u.send(`🎁 Você recebeu **${prod.name}** em **${g.name}**!\n> Motivo: ${reason}`); } catch {} return i.reply({ content: `✅ Produto injetado.`, flags: EPHEMERAL }); }
