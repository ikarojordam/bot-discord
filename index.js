...
async function setupServer(guild, type) {
  ...
  if (!botMember.permissions.has(PermissionFlagsBits.Administrator)) {
    throw new Error('O bot precisa de permissão de Administrador para criar a estrutura.');
  }
  ...
  // Apagar cargos existentes (exceto @everyone e o cargo do bot)
  ...
  // Categoria de vendas (se aplicável)
  ...
  // Enviar painel de ticket no canal de suporte
  if (ticketChannel) {
    const config = await getConfig(guild.id);
    const embed = new EmbedBuilder()
      .setColor('#9B59B6')
      .setTitle(config.ticket_titulo || 'Central de Suporte')
      .setDescription(config.ticket_descricao || 'Clique no botão para abrir ticket.');
  ...
  // Garantir que o @everyone não perca permissões básicas
...
function getCommands() {
  return [
    ...
    new SlashCommandBuilder()
      ...
      .addSubcommand(sub => sub.setName('canais').setDescription('Configurar canais'))
      .addSubcommand(sub => sub.setName('cargos').setDescription('Configurar cargos'))
      .addSubcommand(sub => sub.setName('textos').setDescription('Configurar textos e botões'))
      .addSubcommand(sub => sub.setName('premium').setDescription('Configurar premium'))
    ...
    new SlashCommandBuilder()
      .setName('sorteio')
      .setDescription('Gerencia sorteios (premium)')
      .addSubcommand(sub => sub.setName('criar').setDescription('Cria um novo sorteio')
        ...
        .addIntegerOption(o => o.setName('vencedores').setDescription('Número de vencedores').setRequired(false).setMinValue(1).setMaxValue(10))
        .addChannelOption(o => o.setName('canal').setDescription('Canal para enviar o sorteio').setRequired(false))
        .addStringOption(o => o.setName('descricao').setDescription('Descrição adicional').setRequired(false)))
      .addSubcommand(sub => sub.setName('encerrar').setDescription('Encerra um sorteio manualmente')
        .addStringOption(o => o.setName('message_id').setDescription('ID da mensagem do sorteio').setRequired(true)))
      .setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (commandName === 'configurar') {
      ...
      if (sub === 'canais') {
        const channelOptions = guild.channels.cache
          ...
          .map(c => new StringSelectMenuOptionBuilder().setLabel(c.name).setValue(`channel:${c.id}`))
          .slice(0, 25);
        const select = new StringSelectMenuBuilder()
          .setCustomId('select_config_channel')
          .setPlaceholder('Selecione o canal do painel de vendas')
          .addOptions(channelOptions);
        await interaction.reply({ content: 'Escolha o canal para o painel de vendas:', components: [new ActionRowBuilder().addComponents(select)], ephemeral: true });
        return;
      }
      if (sub === 'cargos') {
        const roleOptions = guild.roles.cache
          .filter(r => r.id !== guild.roles.everyone.id)
          .map(r => new StringSelectMenuOptionBuilder().setLabel(r.name).setValue(`role:${r.id}`))
          .slice(0, 25);
        const selectAdmin = new StringSelectMenuBuilder().setCustomId('select_config_admin_role').setPlaceholder('Selecione o cargo de administrador').addOptions(roleOptions);
        const selectMembro = new StringSelectMenuBuilder().setCustomId('select_config_membro_role').setPlaceholder('Selecione o cargo de membro').addOptions(roleOptions);
        const selectTicket = new StringSelectMenuBuilder().setCustomId('select_config_ticket_role').setPlaceholder('Selecione o cargo de suporte').addOptions(roleOptions);
        await interaction.reply({
          content: 'Selecione os cargos:',
          components: [
            new ActionRowBuilder().addComponents(selectAdmin),
            new ActionRowBuilder().addComponents(selectMembro),
            new ActionRowBuilder().addComponents(selectTicket)
          ],
          ephemeral: true
        });
        return;
      ...
      if (sub === 'textos') {
        ...
        const modal = new ModalBuilder().setCustomId('modal_config_textos').setTitle('Configurar Textos');
        const tituloPainel = new TextInputBuilder().setCustomId('input_painel_titulo').setLabel('Título do painel de vendas').setStyle(TextInputStyle.Short).setRequired(false);
        const descPainel = new TextInputBuilder().setCustomId('input_painel_descricao').setLabel('Descrição do painel de vendas').setStyle(TextInputStyle.Paragraph).setRequired(false);
        const tituloTicket = new TextInputBuilder().setCustomId('input_ticket_titulo').setLabel('Título do ticket').setStyle(TextInputStyle.Short).setRequired(false);
        const descTicket = new TextInputBuilder().setCustomId('input_ticket_descricao').setLabel('Descrição do ticket').setStyle(TextInputStyle.Paragraph).setRequired(false);
        modal.addComponents(
          new ActionRowBuilder().addComponents(tituloPainel),
          new ActionRowBuilder().addComponents(descPainel),
          new ActionRowBuilder().addComponents(tituloTicket),
          new ActionRowBuilder().addComponents(descTicket)
        );
        await interaction.showModal(modal);
        return;
      }
      if (sub === 'premium') {
        // Apenas dev pode ativar premium
        if (!isDeveloper(interaction.user.id)) return interaction.reply({ content: '❌ Apenas o desenvolvedor pode ativar premium.', ephemeral: true });
        const config = await getConfig(guild.id);
        config.is_premium = !config.is_premium;
        await setConfig(guild.id, config);
        await interaction.reply({ content: `✅ Premium ${config.is_premium ? 'ativado' : 'desativado'}.`, ephemeral: true });
        return;
      }
    }
    ...
    if (commandName === 'configurar_ticket') {
      if (!await isAdmin(member, guild)) return interaction.reply({ content: '❌ Sem permissão.', ephemeral: true });
      const roleOptions = guild.roles.cache.filter(r => r.id !== guild.roles.everyone.id).map(r => new StringSelectMenuOptionBuilder().setLabel(r.name).setValue(`role:${r.id}`)).slice(0, 25);
      const channelOptions = guild.channels.cache.filter(c => c.type === ChannelType.GuildText).map(c => new StringSelectMenuOptionBuilder().setLabel(c.name).setValue(`channel:${c.id}`)).slice(0, 25);
      const selectRole = new StringSelectMenuBuilder().setCustomId('select_config_ticket_role').setPlaceholder('Cargo de suporte').addOptions(roleOptions);
      const selectChannel = new StringSelectMenuBuilder().setCustomId('select_config_ticket_log_channel').setPlaceholder('Canal de logs de ticket').addOptions(channelOptions);
      await interaction.reply({
        content: 'Configure o sistema de ticket:',
        components: [
          new ActionRowBuilder().addComponents(selectRole),
    ...
    if (commandName === 'configurar_verificacao') {
      ...
      await setConfig(guild.id, config);
      await interaction.reply({ content: '✅ Configurações de verificação atualizadas!', ephemeral: true });
      return;
    }
    ...
    if (commandName === 'sorteio') {
      ...
      if (sub === 'criar') {
        ...
        await supabase.from('giveaways').insert({
          ...
          ends_at: new Date(Date.now() + duracao * 60000).toISOString(),
          participants: '[]',
          ended: false
        });
        ...
        if (!data || data.ended) return interaction.reply({ content: '❌ Sorteio não encontrado ou já encerrado.', ephemeral: true });
    ...
    if (commandName === 'pix') {
      ...
      if (sub === 'configurar') {
        ...
        if (!qrBuffer) return interaction.reply({ content: '❌ Erro ao gerar QR Code.', ephemeral: true });
        const attachment = new AttachmentBuilder(qrBuffer, { name: 'pix.png' });
        const embed = new EmbedBuilder()
          .setTitle('💠 Pagamento via Pix')
          .setDescription(`**Chave:** \`${config.pix_key}\`\n**Nome:** ${config.pix_nome || 'N/A'}\n**Cidade:** ${config.pix_cidade || 'N/A'}`)
    ...
    if (commandName === 'criar_servidor') {
      ...
      try {
        ...
        await interaction.editReply({ content: `❌ Erro ao criar estrutura do servidor: ${error.message}` });
      }
      return;
    }
  ...
  if (interaction.isStringSelectMenu()) {
    ...
    if (customId === 'select_config_channel') {
      const channelId = interaction.values[0].replace('channel:', '');
      const config = await getConfig(guild.id);
      config.painel_channel = channelId;
      await setConfig(guild.id, config);
      await interaction.update({ content: '✅ Canal do painel definido!', components: [] });
      return;
    }
    if (customId === 'select_config_admin_role') {
      const roleId = interaction.values[0].replace('role:', '');
      const config = await getConfig(guild.id);
      config.admin_role = roleId;
      await setConfig(guild.id, config);
      await interaction.update({ content: '✅ Cargo admin definido!', components: [] });
      return;
    }
    if (customId === 'select_config_membro_role') {
      const roleId = interaction.values[0].replace('role:', '');
      const config = await getConfig(guild.id);
      config.membro_role = roleId;
      await setConfig(guild.id, config);
      await interaction.update({ content: '✅ Cargo de membro definido!', components: [] });
      return;
    }
    if (customId === 'select_config_ticket_role') {
      const roleId = interaction.values[0].replace('role:', '');
      const config = await getConfig(guild.id);
      config.ticket_cargo = roleId;
      await setConfig(guild.id, config);
      await interaction.update({ content: '✅ Cargo de suporte definido!', components: [] });
      return;
    }
    if (customId === 'select_config_ticket_log_channel') {
      const channelId = interaction.values[0].replace('channel:', '');
      const config = await getConfig(guild.id);
      config.ticket_log_channel = channelId;
      await setConfig(guild.id, config);
      await interaction.update({ content: '✅ Canal de logs de ticket definido!', components: [] });
      return;
    }
  }
    ...
    if (interaction.customId === 'btn_abrir_ticket') {
      try {
        ...
        if (!canal || !canal.isTextBased()) {
          return interaction.followUp({ content: '❌ Este canal não suporta tickets.', ephemeral: true });
        }
        if (!canal.permissionsFor(interaction.guild.members.me).has(PermissionFlagsBits.CreatePrivateThreads)) {
          return interaction.followUp({ content: '❌ O bot não tem permissão para criar threads privadas neste canal.', ephemeral: true });
        }
        ...
        await interaction.followUp({ content: '❌ Erro ao criar ticket. Verifique as permissões do bot.', ephemeral: true });
      }
      return;
    }
  ...
  if (interaction.isModalSubmit()) {
    ...
    if (interaction.customId === 'modal_config_textos') {
      const tituloPainel = interaction.fields.getTextInputValue('input_painel_titulo');
      const descPainel = interaction.fields.getTextInputValue('input_painel_descricao');
      const tituloTicket = interaction.fields.getTextInputValue('input_ticket_titulo');
      const descTicket = interaction.fields.getTextInputValue('input_ticket_descricao');
      const config = await getConfig(guild.id);
      if (tituloPainel) config.painel_titulo = tituloPainel;
      if (descPainel) config.painel_descricao = descPainel;
      if (tituloTicket) config.ticket_titulo = tituloTicket;
      if (descTicket) config.ticket_descricao = descTicket;
      await setConfig(guild.id, config);
      await interaction.editReply({ content: '✅ Textos atualizados!' });
      return;
    }
  }
});
