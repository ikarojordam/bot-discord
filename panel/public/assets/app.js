...
const state = { user: null, admin: null, role: null, pollTimer: null, currentPage: 'dashboard', notifications: [], currentServer: null };
...
async function boot() {
  ...
  const rt = hash.get('access_token');
  if (rt) { window.__RESET_TOKEN__ = rt; showView('reset'); return; }
  const token = localStorage.getItem('sb_token');
...
async function loadKeys() {
  ...
  try {
    ...
    t.innerHTML = `<table><thead><tr><th>Key</th><th>Tier</th><th>Duração</th><th>Usos</th><th>Status</th><th>Criada</th><th></th></tr></thead>
    ...
    $('#serverDetail').innerHTML = `
      ...
      <div class="detail-panel" id="dt-actions">
        <div class="card">
          ...
          <div class="field"><label>Quantidade (máx 50)</label><input type="number" id="tmLimit" value="20" min="1" max="50"></div>
...
async function loadServerTickets(gid) {
  ...
  try {
    ...
    el.innerHTML = `<table><thead><tr><th>Autor</th><th>Status</th><th>Aberto em</th></tr></thead>
      <tbody>${tickets.map(t => `<tr><td>${escapeHtml(t.user_id)}</td><td>${t.closed_at ? '🔴 Fechado' : '🟢 Aberto'}</td><td>${fmtDate(t.opened_at)}</td></tr>`).join('')}</tbody></table>`;
...
async function loadServerProducts(gid) {
  ...
  try {
    ...
    el.innerHTML = `<table><thead><tr><th>Nome</th><th>Preço</th><th>Estoque</th><th>Ativo</th></tr></thead>
      <tbody>${products.map(p => `<tr><td>${escapeHtml(p.name)}</td><td>R$ ${Number(p.price).toFixed(2)}</td><td>${p.infinite_content ? '∞' : '—'}</td><td>${p.active ? '✅' : '❌'}</td></tr>`).join('')}</tbody></table>`;
  } catch (e) { el.innerHTML = `<div class="empty" style="color:#f87171">❌ ${escapeHtml(e.message)}</div>`; }
}
async function loadServerOrders(gid) {
  ...
  try {
    ...
    el.innerHTML = `<table><thead><tr><th>#</th><th>Cliente</th><th>Total</th><th>Status</th></tr></thead>
      <tbody>${orders.map(o => `<tr><td>${o.id}</td><td>${escapeHtml(o.user_id)}</td><td>R$ ${Number(o.total || 0).toFixed(2)}</td><td>${escapeHtml(o.status)}</td></tr>`).join('')}</tbody></table>`;
  } catch (e) { el.innerHTML = `<div class="empty" style="color:#f87171">❌ ${escapeHtml(e.message)}</div>`; }
}
async function takeMembers() {
  const gid = state.currentServer;
  const limit = Number($('#tmLimit').value) || 20;
  const btn = $('#btnTakeMembers'); btn.disabled = true; btn.textContent = '⏳ Levando...';
  const res = $('#tmResult'); res.className = 'msg info'; res.textContent = `Processando ${limit} usuários... (pode levar ~${limit}s)`;
  try {
    const r = await api(`/api/me/servers/${gid}/take-members`, { method: 'POST', body: JSON.stringify({ limit }) });
    res.className = 'msg ok'; res.textContent = `✅ Adicionados: ${r.added} | ❌ Falhas: ${r.failed} | Total: ${r.total}`;
...
async function loadNotifications() {
  try {
    ...
    state.notifications = notifications; updateBell(unread); renderNotifDrawer(notifications);
    if (state.currentPage === 'notifications') renderNotifPage();
  } catch {}
}
function updateBell(n) {
  const b = $('#bellBadge'), nav = $('#navNotifBadge');
  if (n > 0) {
...
async function loadUsers() {
  ...
  try {
    ...
    t.innerHTML = `<table><thead><tr><th>Email</th><th>Nome</th><th>Role</th><th>Discord</th><th>Servidores</th><th>Ativo</th><th>Ações</th></tr></thead>
      <tbody>${r.usuarios.map(u => `<tr>
...
function editUser(u) {
  ...
  $('#modalUserId').value = u.user_id; $('#modalUserEmail').value = u.email; $('#modalUserEmail').disabled = true;
  $('#modalUserDiscord').value = u.discord_id || ''; $('#modalUserNome').value = u.nome || '';
  $('#modalUserRole').value = u.role; $('#fieldPassword').style.display = 'none';
  $('#modalUserGuilds').value = (u.assigned_guilds || []).join(', '); $('#modalUserNotes').value = u.notes || '';
...
$('#btnNovoUsuario')?.addEventListener('click', () => {
  ...
  $('#fieldPassword').style.display = ''; $('#btnSaveUser').textContent = 'Criar'; $('#userForm').dataset.mode = 'create'; openModal('modalUser');
});
$('#userForm')?.addEventListener('submit', async e => {
  ...
  try {
    const payload = {
      email: $('#modalUserEmail').value.trim(), discord_id: $('#modalUserDiscord').value.trim(),
      nome: $('#modalUserNome').value.trim() || null, role: $('#modalUserRole').value,
...
function openApprove(u) {
  ...
  $('#approveRole').value = 'cliente'; $('#approveGuilds').value = ''; $('#approveNotes').value = ''; openModal('modalApprove');
}
...
$('#btnKillOn')?.addEventListener('click', async () => {
  ...
  try { await api('/api/dev/kill-switch', { method: 'POST', body: JSON.stringify({ active: true, reason: $('#ksReason').value }) }); toast('Kill switch ON'); loadKillSwitch(); }
  catch (e) { toast(e.message, 'error'); }
});
$('#btnKillOff')?.addEventListener('click', async () => {
  try { await api('/api/dev/kill-switch', { method: 'POST', body: JSON.stringify({ active: false }) }); toast('Kill switch OFF'); loadKillSwitch(); }
  catch (e) { toast(e.message, 'error'); }
});
...
$('#btnMtOn')?.addEventListener('click', async () => {
  ...
  try { await api('/api/dev/maintenance', { method: 'POST', body: JSON.stringify({ active: true, reason: $('#mtReason').value }) }); toast('Manutenção ON'); loadMaintenance(); }
  catch (e) { toast(e.message, 'error'); }
});
$('#btnMtOff')?.addEventListener('click', async () => {
  try { await api('/api/dev/maintenance', { method: 'POST', body: JSON.stringify({ active: false }) }); toast('Manutenção OFF'); loadMaintenance(); }
  catch (e) { toast(e.message, 'error'); }
});
...
async function loadForcePremium() {
  ...
  try {
    ...
    t.innerHTML = `<table><thead><tr><th>Scope</th><th>Target</th><th>Tipo</th><th>Por</th><th>Quando</th><th></th></tr></thead>
...
$('#btnBcSend')?.addEventListener('click', async () => {
  ...
  try {
    ...
    toast(`Enviado para ${r.sent}`); $('#bcTitle').value = ''; $('#bcContent').value = '';
  } catch (e) { toast(e.message, 'error'); }
});
...
$('#registerForm')?.addEventListener('submit', async e => {
  ...
  try {
    const r = await api('/api/auth/register', { method: 'POST', body: JSON.stringify({
      email: $('#regEmail').value.trim(), password: p1, discord_id: $('#regDiscord').value.trim() || null,
    })});
    showMsg(msg, '✅ ' + (r.message || 'Enviado!'), 'ok');
...
$('#btnSendForgot')?.addEventListener('click', async () => {
  ...
  if (!email) return toast('E-mail vazio', 'error');
...
$('#resetForm')?.addEventListener('submit', async e => {
  ...
  try {
    await api('/api/auth/update-password', { method: 'POST', body: JSON.stringify({ token: window.__RESET_TOKEN__, new_password: p1 }) });
    showMsg(msg, '✅ OK! Redirecionando…', 'ok');
